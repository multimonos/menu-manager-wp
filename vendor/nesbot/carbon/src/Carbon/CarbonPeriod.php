...
use MenuManager\Vendor\Carbon\Exceptions\EndLessPeriodException;
...
use MenuManager\Vendor\Carbon\Exceptions\InvalidPeriodDateException;
use MenuManager\Vendor\Carbon\Exceptions\InvalidPeriodParameterException;
use MenuManager\Vendor\Carbon\Exceptions\NotACarbonClassException;
use MenuManager\Vendor\Carbon\Exceptions\NotAPeriodException;
use MenuManager\Vendor\Carbon\Exceptions\UnknownGetterException;
use MenuManager\Vendor\Carbon\Exceptions\UnknownMethodException;
use MenuManager\Vendor\Carbon\Exceptions\UnreachableException;
...
class CarbonPeriod implements Iterator, Countable, JsonSerializable
{
     ...
     * @var string
     */
    protected const DEFAULT_DATE_CLASS = \MenuManager\Vendor\Carbon\Carbon::class;
    /**
     ...
     * @var string
     */
    protected $dateClass = \MenuManager\Vendor\Carbon\Carbon::class;
    /**
    ...
    protected static function parseIso8601($iso)
    {
        ...
        foreach (\explode('/', $iso) as $key => $part) {
            ...
            } elseif ($interval === null && ($parsed = \MenuManager\Vendor\Carbon\CarbonInterval::make($part))) {
                $interval = $part;
    ...
    public function __construct(...$arguments)
    {
        ...
        foreach ($arguments as $argument) {
            ...
            } elseif ($this->dateInterval === null && (\is_string($argument) && \preg_match('/^(-?\\d(\\d(?![\\/-])|[^\\d\\/-]([\\/-])?)*|P[T\\d].*|(?:\\h*\\d+(?:\\.\\d+)?\\h*[a-z]+)+)$/i', $argument) || $argument instanceof DateInterval || $argument instanceof Closure) && ($parsedInterval = @\MenuManager\Vendor\Carbon\CarbonInterval::make($argument))) {
        ...
        if ($this->dateInterval === null) {
            $this->setDateInterval(\MenuManager\Vendor\Carbon\CarbonInterval::day());
    ...
    {
        if (!\is_a($dateClass, \MenuManager\Vendor\Carbon\CarbonInterface::class, \true)) {
            throw new NotACarbonClassException($dateClass);
        }
        $self = $this->copyIfImmutable();
        $self->dateClass = $dateClass;
        if (\is_a($dateClass, \MenuManager\Vendor\Carbon\Carbon::class, \true)) {
            $self->options = $self->options & ~static::IMMUTABLE;
        } elseif (\is_a($dateClass, \MenuManager\Vendor\Carbon\CarbonImmutable::class, \true)) {
            $self->options = $self->options | static::IMMUTABLE;
        }
        return $self;
    }
    ...
    {
        if (!($interval = \MenuManager\Vendor\Carbon\CarbonInterval::make($interval))) {
    ...
    public function __call($method, $parameters)
    {
        ...
        switch ($method) {
            ...
            case 'sinceNow':
                return $this->setStartDate(new \MenuManager\Vendor\Carbon\Carbon(), ...$parameters);
            case 'end':
            case 'until':
                self::setDefaultParameters($parameters, [[0, 'date', null]]);
                return $this->setEndDate(...$parameters);
            case 'untilNow':
                return $this->setEndDate(new \MenuManager\Vendor\Carbon\Carbon(), ...$parameters);
            case 'dates':
            case 'between':
            ...
            case 'microseconds':
            case 'microsecond':
                return $this->setDateInterval([$this->isDefaultInterval ? new \MenuManager\Vendor\Carbon\CarbonInterval('PT0S') : $this->dateInterval, $method](...$parameters));
        }
        $dateClass = $this->dateClass;
    ...
    private function getEndFromRecurrences()
    {
        ...
        if ($this->recurrences === \INF) {
            $start = $this->getStartDate();
            return $start < $start->avoidMutation()->add($this->getDateInterval()) ? \MenuManager\Vendor\Carbon\CarbonImmutable::endOfTime() : \MenuManager\Vendor\Carbon\CarbonImmutable::startOfTime();
        }
    ...
    {
        return $this->roundWith($precision ?? $this->getDateInterval()->setLocalTranslator(\MenuManager\Vendor\Carbon\TranslatorImmutable::get('en'))->forHumans(), $function);
    }
    /**
     * Round the current instance second with given precision if specified (else period interval is used).
     *
    ...
    {
        if ($this->getOptions() & static::IMMUTABLE && $this->dateClass === \MenuManager\Vendor\Carbon\Carbon::class) {
            $this->dateClass = \MenuManager\Vendor\Carbon\CarbonImmutable::class;
        } elseif (!($this->getOptions() & static::IMMUTABLE) && $this->dateClass === \MenuManager\Vendor\Carbon\CarbonImmutable::class) {
            $this->dateClass = \MenuManager\Vendor\Carbon\Carbon::class;
        }
        $this->validationResult = null;
    }
    /**
     ...
     * @return CarbonInterface
     */
    protected function prepareForReturn(\MenuManager\Vendor\Carbon\CarbonInterface $date)
    {
    ...
    {
        return $date instanceof \MenuManager\Vendor\Carbon\CarbonInterface && ($date->isEndOfTime() || $date->isStartOfTime());
    }
    private function rawDate($date) : ?DateTimeInterface
    {
        ...
        if ($date instanceof \MenuManager\Vendor\Carbon\CarbonInterface) {
