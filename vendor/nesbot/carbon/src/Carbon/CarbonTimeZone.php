...
use MenuManager\Vendor\Carbon\Exceptions\InvalidTimeZoneException;
...
class CarbonTimeZone extends DateTimeZone
{
    ...
    {
        return static::getOffsetNameFromMinuteOffset($this->getOffset($date ?: \MenuManager\Vendor\Carbon\Carbon::now($this)) / 60);
    }
    /**
    ...
    public function toRegionName(?DateTimeInterface $date = null, $isDst = 1)
    {
        ...
        $date = $date ?: \MenuManager\Vendor\Carbon\Carbon::now($this);
        ...
        foreach (\timezone_identifiers_list() as $timezone) {
            if (\MenuManager\Vendor\Carbon\Carbon::instance($date)->tz($timezone)->getOffset() === $offset) {
                return $timezone;
            }
        }
        return \false;
    ...
    public function toRegionTimeZone(?DateTimeInterface $date = null)
    {
        ...
        if (\MenuManager\Vendor\Carbon\Carbon::isStrictModeEnabled()) {
            throw new InvalidTimeZoneException('Unknown timezone for offset ' . $this->getOffset($date ?: \MenuManager\Vendor\Carbon\Carbon::now($this)) . ' seconds.');
        }
        return \false;
    }
    /**
    ...
    {
        return static::createFromMinuteOffset($hourOffset * \MenuManager\Vendor\Carbon\Carbon::MINUTES_PER_HOUR);
    }
    /**
