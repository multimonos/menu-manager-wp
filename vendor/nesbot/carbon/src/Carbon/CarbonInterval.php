...
use MenuManager\Vendor\Carbon\Exceptions\BadFluentConstructorException;
use MenuManager\Vendor\Carbon\Exceptions\BadFluentSetterException;
use MenuManager\Vendor\Carbon\Exceptions\InvalidCastException;
use MenuManager\Vendor\Carbon\Exceptions\InvalidIntervalException;
use MenuManager\Vendor\Carbon\Exceptions\OutOfRangeException;
use MenuManager\Vendor\Carbon\Exceptions\ParseErrorException;
use MenuManager\Vendor\Carbon\Exceptions\UnitNotConfiguredException;
use MenuManager\Vendor\Carbon\Exceptions\UnknownGetterException;
use MenuManager\Vendor\Carbon\Exceptions\UnknownSetterException;
use MenuManager\Vendor\Carbon\Exceptions\UnknownUnitException;
use MenuManager\Vendor\Carbon\Traits\IntervalRounding;
use MenuManager\Vendor\Carbon\Traits\IntervalStep;
use MenuManager\Vendor\Carbon\Traits\MagicParameter;
use MenuManager\Vendor\Carbon\Traits\Mixin;
use MenuManager\Vendor\Carbon\Traits\Options;
use MenuManager\Vendor\Carbon\Traits\ToStringFormat;
use Closure;
use DateInterval;
use MenuManager\Vendor\DateMalformedIntervalStringException;
 ...
 */
class CarbonInterval extends DateInterval implements \MenuManager\Vendor\Carbon\CarbonConverterInterface
{
    ...
    {
        return ['milliseconds' => [\MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_MILLISECOND, 'microseconds'], 'seconds' => [\MenuManager\Vendor\Carbon\Carbon::MILLISECONDS_PER_SECOND, 'milliseconds'], 'minutes' => [\MenuManager\Vendor\Carbon\Carbon::SECONDS_PER_MINUTE, 'seconds'], 'hours' => [\MenuManager\Vendor\Carbon\Carbon::MINUTES_PER_HOUR, 'minutes'], 'dayz' => [\MenuManager\Vendor\Carbon\Carbon::HOURS_PER_DAY, 'hours'], 'weeks' => [\MenuManager\Vendor\Carbon\Carbon::DAYS_PER_WEEK, 'dayz'], 'months' => [\MenuManager\Vendor\Carbon\Carbon::WEEKS_PER_MONTH, 'weeks'], 'years' => [\MenuManager\Vendor\Carbon\Carbon::MONTHS_PER_YEAR, 'months']];
    }
    ...
    public function __construct($years = 1, $months = null, $weeks = null, $days = null, $hours = null, $minutes = null, $seconds = null, $microseconds = null)
    {
        ...
        if ($microseconds !== null) {
            $this->f = $microseconds / \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_SECOND;
        }
    }
    /**
    ...
    public static function getFactorWithDefault($source, $target)
    {
        ...
        static $defaults = ['month' => ['year' => \MenuManager\Vendor\Carbon\Carbon::MONTHS_PER_YEAR], 'week' => ['month' => \MenuManager\Vendor\Carbon\Carbon::WEEKS_PER_MONTH], 'day' => ['week' => \MenuManager\Vendor\Carbon\Carbon::DAYS_PER_WEEK], 'hour' => ['day' => \MenuManager\Vendor\Carbon\Carbon::HOURS_PER_DAY], 'minute' => ['hour' => \MenuManager\Vendor\Carbon\Carbon::MINUTES_PER_HOUR], 'second' => ['minute' => \MenuManager\Vendor\Carbon\Carbon::SECONDS_PER_MINUTE], 'millisecond' => ['second' => \MenuManager\Vendor\Carbon\Carbon::MILLISECONDS_PER_SECOND], 'microsecond' => ['millisecond' => \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_MILLISECOND]];
    ...
    {
        return static::getFactor('dayz', 'weeks') ?: \MenuManager\Vendor\Carbon\Carbon::DAYS_PER_WEEK;
    }
    /**
    ...
    {
        return static::getFactor('hours', 'dayz') ?: \MenuManager\Vendor\Carbon\Carbon::HOURS_PER_DAY;
    }
    /**
    ...
    {
        return static::getFactor('minutes', 'hours') ?: \MenuManager\Vendor\Carbon\Carbon::MINUTES_PER_HOUR;
    }
    /**
    ...
    {
        return static::getFactor('seconds', 'minutes') ?: \MenuManager\Vendor\Carbon\Carbon::SECONDS_PER_MINUTE;
    }
    /**
    ...
    {
        return static::getFactor('milliseconds', 'seconds') ?: \MenuManager\Vendor\Carbon\Carbon::MILLISECONDS_PER_SECOND;
    }
    /**
     * Returns current config for microseconds per second.
     *
    ...
    {
        return static::getFactor('microseconds', 'milliseconds') ?: \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_MILLISECOND;
    }
    /**
    ...
    public static function __callStatic($method, $parameters)
    {
        try {
            ...
            if (\MenuManager\Vendor\Carbon\Carbon::isStrictModeEnabled()) {
    ...
    public static function fromString($intervalDefinition)
    {
        ...
        while ([$part, $value, $unit] = \array_shift($parts)) {
            ...
            switch ($unit === 'µs' ? 'µs' : \strtolower($unit)) {
                case 'millennia':
                case 'millennium':
                    $years += $intValue * \MenuManager\Vendor\Carbon\CarbonInterface::YEARS_PER_MILLENNIUM;
                    break;
                case 'century':
                case 'centuries':
                    $years += $intValue * \MenuManager\Vendor\Carbon\CarbonInterface::YEARS_PER_CENTURY;
                    break;
                case 'decade':
                case 'decades':
                    $years += $intValue * \MenuManager\Vendor\Carbon\CarbonInterface::YEARS_PER_DECADE;
                    break;
                case 'year':
                case 'years':
                case 'y':
                ...
                case 'quarter':
                case 'quarters':
                    $months += $intValue * \MenuManager\Vendor\Carbon\CarbonInterface::MONTHS_PER_QUARTER;
                    break;
                case 'month':
                case 'months':
                case 'mo':
        ...
        return new static($years, $months, $weeks, $days, $hours, $minutes, $seconds, $milliseconds * \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_MILLISECOND + $microseconds);
    }
    /**
    ...
    {
        return static::fromString(\MenuManager\Vendor\Carbon\Carbon::translateTimeString($interval, $locale ?: static::getLocale(), 'en'));
    }
    ...
    {
        if ($unit) {
            $interval = "{$interval} " . \MenuManager\Vendor\Carbon\Carbon::pluralUnit($unit);
        }
    ...
    public function get($name)
    {
        ...
        switch ($name) {
            ...
            case 'milli':
            case 'milliseconds':
                return (int) (\round($this->f * \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_SECOND) / \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_MILLISECOND);
            case 'micro':
            case 'microseconds':
                return (int) \round($this->f * \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_SECOND);
    ...
    public function set($name, $value = null)
    {
        ...
        foreach ($properties as $key => $value) {
            switch (\MenuManager\Vendor\Carbon\Carbon::singularUnit(\rtrim($key, 'z'))) {
                case 'year':
                ...
                case 'milli':
                case 'millisecond':
                    $this->microseconds = $value * \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_MILLISECOND + $this->microseconds % \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_MILLISECOND;
                    break;
                case 'micro':
                case 'microsecond':
                    $this->f = $value / \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_SECOND;
                    break;
                default:
                    if ($this->localStrictModeEnabled ?? \MenuManager\Vendor\Carbon\Carbon::isStrictModeEnabled()) {
    ...
    public function __call($method, $parameters)
    {
        ...
        if (\preg_match('/^(?<method>add|sub)(?<unit>[A-Z].*)$/', $method, $match)) {
            $value = $this->getMagicParameter($parameters, 0, \MenuManager\Vendor\Carbon\Carbon::pluralUnit($match['unit']), 0);
            return $this->{$match['method']}($value, $match['unit']);
        }
        $value = $this->getMagicParameter($parameters, 0, \MenuManager\Vendor\Carbon\Carbon::pluralUnit($method), 1);
        try {
    ...
    protected function getForHumansInitialVariables($syntax, $short)
    {
        ...
        if (\is_bool($syntax)) {
            return ['short' => $syntax, 'syntax' => \MenuManager\Vendor\Carbon\CarbonInterface::DIFF_ABSOLUTE];
        }
        return [];
    }
    /**
    ...
    protected function getForHumansParameters($syntax = null, $short = \false, $parts = -1, $options = null)
    {
        ...
        if ($syntax === null) {
            $syntax = \MenuManager\Vendor\Carbon\CarbonInterface::DIFF_ABSOLUTE;
        }
        if ($parts === -1) {
            $parts = \INF;
        }
        ...
        if ($altNumbers && $altNumbers !== \true) {
            $language = new \MenuManager\Vendor\Carbon\Language($this->locale);
    ...
    {
        if ($options & \MenuManager\Vendor\Carbon\CarbonInterface::ROUND) {
            return 'round';
        }
        if ($options & \MenuManager\Vendor\Carbon\CarbonInterface::CEIL) {
            return 'ceil';
        }
        if ($options & \MenuManager\Vendor\Carbon\CarbonInterface::FLOOR) {
            return 'floor';
        }
        return null;
    }
    ...
    public function forHumans($syntax = null, $short = \false, $parts = -1, $options = null)
    {
        ...
        $interval = [];
        $syntax = (int) ($syntax ?? \MenuManager\Vendor\Carbon\CarbonInterface::DIFF_ABSOLUTE);
        $absolute = $syntax === \MenuManager\Vendor\Carbon\CarbonInterface::DIFF_ABSOLUTE;
        $relativeToNow = $syntax === \MenuManager\Vendor\Carbon\CarbonInterface::DIFF_RELATIVE_TO_NOW;
        $count = 1;
        $unit = $short ? 's' : 'second';
        ...
        foreach ($diffIntervalArray as $diffIntervalData) {
            ...
            } elseif ($options & \MenuManager\Vendor\Carbon\CarbonInterface::SEQUENTIAL_PARTS_ONLY && \count($interval) > 0) {
                break;
            }
        ...
        if (\count($interval) === 0) {
            if ($relativeToNow && $options & \MenuManager\Vendor\Carbon\CarbonInterface::JUST_NOW) {
                $key = 'diff_now';
            ...
            $count = $options & \MenuManager\Vendor\Carbon\CarbonInterface::NO_ZERO_DIFF ? 1 : 0;
        ...
        if ($parts === 1) {
            if ($relativeToNow && $unit === 'day') {
                if ($count === 1 && $options & \MenuManager\Vendor\Carbon\CarbonInterface::ONE_DAY_WORDS) {
                ...
                if ($count === 2 && $options & \MenuManager\Vendor\Carbon\CarbonInterface::TWO_DAY_WORDS) {
    ...
    public function toPeriod(...$params)
    {
        ...
        return \MenuManager\Vendor\Carbon\CarbonPeriod::create($this, ...$params);
    }
    /**
     * Invert the interval.
     *
    ...
    public static function getDateIntervalSpec(DateInterval $interval, bool $microseconds = \false, array $skip = [])
    {
        ...
        if ($interval->days >= \MenuManager\Vendor\Carbon\CarbonInterface::DAYS_PER_WEEK * \MenuManager\Vendor\Carbon\CarbonInterface::WEEKS_PER_MONTH && (!isset($date[static::PERIOD_YEARS]) || \count(\array_intersect(['y', 'year', 'years'], $skip))) && (!isset($date[static::PERIOD_MONTHS]) || \count(\array_intersect(['m', 'month', 'months'], $skip)))) {
    ...
    {
        $current = \MenuManager\Vendor\Carbon\Carbon::now();
    ...
    public function total($unit)
    {
        ...
        $values = ['years' => $this->years, 'months' => $this->months, 'weeks' => (int) ($this->d / $daysPerWeek), 'dayz' => \fmod($this->d, $daysPerWeek), 'hours' => $this->hours, 'minutes' => $this->minutes, 'seconds' => $this->seconds, 'milliseconds' => (int) ($this->microseconds / \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_MILLISECOND), 'microseconds' => $this->microseconds % \MenuManager\Vendor\Carbon\Carbon::MICROSECONDS_PER_MILLISECOND];
    ...
    public function roundUnit($unit, $precision = 1, $function = 'round')
    {
        ...
        $base = \MenuManager\Vendor\Carbon\CarbonImmutable::parse('2000-01-01 00:00:00', 'UTC')->roundUnit($unit, $precision, $function);
        $next = $base->add($this);
        $inverted = $next < $base;
        if ($inverted) {
            $next = $base->sub($this);
