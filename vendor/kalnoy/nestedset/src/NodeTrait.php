...
use MenuManager\Vendor\Illuminate\Database\Eloquent\Relations\BelongsTo;
...
trait NodeTrait
{
    ...
    {
        return new \MenuManager\Vendor\Kalnoy\Nestedset\DescendantsRelation($this->newQuery(), $this);
    }
    /**
    ...
    {
        return $this->newScopedQuery()->where($this->getKeyName(), '<>', $this->getKey())->where($this->getParentIdName(), '=', $this->getParentId());
    }
    /**
    ...
    {
        return $this->newScopedQuery()->where($this->getParentIdName(), '=', $this->getParentId());
    }
    /**
    ...
    {
        return $this->nextNodes()->where($this->getParentIdName(), '=', $this->getParentId());
    }
    /**
    ...
    {
        return $this->prevNodes()->where($this->getParentIdName(), '=', $this->getParentId());
    }
    /**
    ...
    {
        return $this->newScopedQuery()->where($this->getLftName(), '>', $this->getLft());
    }
    /**
    ...
    {
        return new \MenuManager\Vendor\Kalnoy\Nestedset\AncestorsRelation($this->newQuery(), $this);
    }
    /**
    ...
    {
        return $this->appendOrPrependTo($parent, \true);
    }
    /**
     * @param self $parent
     * @param bool $prepend
    ...
    {
        $this->assertNodeExists($parent)->assertNotDescendant($parent)->assertSameScope($parent);
    ...
    {
        return $this->beforeOrAfterNode($node, \true);
    }
    /**
    ...
    {
        $this->assertNodeExists($node)->assertNotDescendant($node)->assertSameScope($node);
    ...
    {
        $sibling = $this->prevSiblings()->defaultOrder('desc')->skip($amount - 1)->first();
        if (!$sibling) {
            return \false;
        }
    ...
    {
        $sibling = $this->nextSiblings()->defaultOrder()->skip($amount - 1)->first();
        if (!$sibling) {
            return \false;
        }
    ...
    {
        $this->descendants()->where($this->getDeletedAtColumn(), '>=', $deletedAt)->restore();
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        return new \MenuManager\Vendor\Kalnoy\Nestedset\QueryBuilder($query);
    }
    /**
    ...
    {
        return new \MenuManager\Vendor\Kalnoy\Nestedset\Collection($models);
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        return \MenuManager\Vendor\Kalnoy\Nestedset\NestedSet::LFT;
    }
    /**
    ...
    {
        return \MenuManager\Vendor\Kalnoy\Nestedset\NestedSet::RGT;
    }
    /**
    ...
    {
        return $this->getLft() > $other->getLft() && $this->getLft() < $other->getRgt() && $this->isSameScope($other);
    }
    /**
    ...
    {
        $defaults = [$this->getParentIdName(), $this->getLftName(), $this->getRgtName()];
