...
class Complex
{
    ...
    private static function parseComplex($complexNumber)
    {
        ...
        if (!$validComplex) {
            ...
            if (!$validComplex) {
                throw new \MenuManager\Vendor\Complex\Exception('Invalid complex number');
            }
     ...
     * @throws    Exception    If the argument isn't a Complex number or cannot be converted to one
     */
    public static function validateComplexArgument($complex) : \MenuManager\Vendor\Complex\Complex
    {
        if (\is_scalar($complex) || \is_array($complex)) {
            $complex = new \MenuManager\Vendor\Complex\Complex($complex);
        } elseif (!\is_object($complex) || !$complex instanceof \MenuManager\Vendor\Complex\Complex) {
            throw new \MenuManager\Vendor\Complex\Exception('Value is not a valid complex number');
        }
        return $complex;
    }
    /**
     ...
     * @return    Complex
     */
    public function reverse() : \MenuManager\Vendor\Complex\Complex
    {
        return new \MenuManager\Vendor\Complex\Complex($this->imaginaryPart, $this->realPart, $this->realPart == 0.0 ? null : $this->suffix);
    }
    public function invertImaginary() : \MenuManager\Vendor\Complex\Complex
    {
        return new \MenuManager\Vendor\Complex\Complex($this->realPart, $this->imaginaryPart * -1, $this->imaginaryPart == 0.0 ? null : $this->suffix);
    }
    public function invertReal() : \MenuManager\Vendor\Complex\Complex
    {
        return new \MenuManager\Vendor\Complex\Complex($this->realPart * -1, $this->imaginaryPart, $this->imaginaryPart == 0.0 ? null : $this->suffix);
    }
    ...
    public function __call($functionName, $arguments)
    {
        ...
        if (\in_array($functionName, self::$functions, \true)) {
            return \MenuManager\Vendor\Complex\Functions::$functionName($this, ...$arguments);
        }
        // Test for operation calls
        if (\in_array($functionName, self::$operations, \true)) {
            return \MenuManager\Vendor\Complex\Operations::$functionName($this, ...$arguments);
        }
        throw new \MenuManager\Vendor\Complex\Exception('Complex Function or Operation does not exist');
    }
}
