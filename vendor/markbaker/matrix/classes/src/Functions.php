...
class Functions
{
    ...
    private static function validateMatrix($matrix)
    {
        ...
        if (!$matrix instanceof \MenuManager\Vendor\Matrix\Matrix) {
            throw new \MenuManager\Vendor\Matrix\Exception('Must be Matrix or array');
        }
        return $matrix;
    }
    /**
     ...
     * @throws Exception
     */
    private static function getAdjoint(\MenuManager\Vendor\Matrix\Matrix $matrix)
    {
    ...
    public static function adjoint($matrix)
    {
        ...
        if (!$matrix->isSquare()) {
            throw new \MenuManager\Vendor\Matrix\Exception('Adjoint can only be calculated for a square matrix');
        }
     ...
     * @throws Exception
     */
    private static function getCofactors(\MenuManager\Vendor\Matrix\Matrix $matrix)
    {
        ...
        return new \MenuManager\Vendor\Matrix\Matrix($cofactors);
    }
    /**
    ...
    public static function cofactors($matrix)
    {
        $matrix = self::validateMatrix($matrix);
        if (!$matrix->isSquare()) {
            throw new \MenuManager\Vendor\Matrix\Exception('Cofactors can only be calculated for a square matrix');
        }
     ...
     * @throws Exception
     */
    private static function getDeterminantSegment(\MenuManager\Vendor\Matrix\Matrix $matrix, $row, $column)
    {
        ...
        return self::getDeterminant(new \MenuManager\Vendor\Matrix\Matrix($tmpMatrix));
    }
    /**
     ...
     * @throws Exception
     */
    private static function getDeterminant(\MenuManager\Vendor\Matrix\Matrix $matrix)
    {
        $dimensions = $matrix->rows;
        $determinant = 0;
        switch ($dimensions) {
    ...
    public static function determinant($matrix)
    {
        $matrix = self::validateMatrix($matrix);
        if (!$matrix->isSquare()) {
            throw new \MenuManager\Vendor\Matrix\Exception('Determinant can only be calculated for a square matrix');
        }
    ...
    public static function diagonal($matrix)
    {
        $matrix = self::validateMatrix($matrix);
        if (!$matrix->isSquare()) {
            throw new \MenuManager\Vendor\Matrix\Exception('Diagonal can only be extracted from a square matrix');
        }
        $dimensions = $matrix->rows;
        $grid = \MenuManager\Vendor\Matrix\Builder::createFilledMatrix(0, $dimensions, $dimensions)->toArray();
        for ($i = 0; $i < $dimensions; ++$i) {
            $grid[$i][$i] = $matrix->getValue($i + 1, $i + 1);
        }
        return new \MenuManager\Vendor\Matrix\Matrix($grid);
    }
    /**
    ...
    public static function antidiagonal($matrix)
    {
        $matrix = self::validateMatrix($matrix);
        if (!$matrix->isSquare()) {
            throw new \MenuManager\Vendor\Matrix\Exception('Anti-Diagonal can only be extracted from a square matrix');
        }
        $dimensions = $matrix->rows;
        $grid = \MenuManager\Vendor\Matrix\Builder::createFilledMatrix(0, $dimensions, $dimensions)->toArray();
        for ($i = 0; $i < $dimensions; ++$i) {
    ...
    public static function identity($matrix)
    {
        $matrix = self::validateMatrix($matrix);
        if (!$matrix->isSquare()) {
            throw new \MenuManager\Vendor\Matrix\Exception('Identity can only be created for a square matrix');
        }
        $dimensions = $matrix->rows;
        return \MenuManager\Vendor\Matrix\Builder::createIdentityMatrix($dimensions);
    }
    /**
    ...
    public static function inverse($matrix, string $type = 'inverse')
    {
        $matrix = self::validateMatrix($matrix);
        if (!$matrix->isSquare()) {
            throw new \MenuManager\Vendor\Matrix\Exception(\ucfirst($type) . ' can only be calculated for a square matrix');
        }
        $determinant = self::getDeterminant($matrix);
        if ($determinant == 0.0) {
            throw new \MenuManager\Vendor\Matrix\Div0Exception(\ucfirst($type) . ' can only be calculated for a matrix with a non-zero determinant');
        }
        if ($matrix->rows == 1) {
            return new \MenuManager\Vendor\Matrix\Matrix([[1 / $matrix->getValue(1, 1)]]);
        }
     ...
     * @throws Exception
     */
    protected static function getMinors(\MenuManager\Vendor\Matrix\Matrix $matrix)
    {
    ...
    public static function minors($matrix)
    {
        $matrix = self::validateMatrix($matrix);
        if (!$matrix->isSquare()) {
            throw new \MenuManager\Vendor\Matrix\Exception('Minors can only be calculated for a square matrix');
        }
        return new \MenuManager\Vendor\Matrix\Matrix(self::getMinors($matrix));
    }
    /**
    ...
    public static function trace($matrix)
    {
        $matrix = self::validateMatrix($matrix);
        if (!$matrix->isSquare()) {
            throw new \MenuManager\Vendor\Matrix\Exception('Trace can only be extracted from a square matrix');
        }
        $dimensions = $matrix->rows;
        $result = 0;
        for ($i = 1; $i <= $dimensions; ++$i) {
