...
use Generator;
use MenuManager\Vendor\Matrix\Decomposition\LU;
use MenuManager\Vendor\Matrix\Decomposition\QR;
/**
 * Matrix object.
 *
 * @package Matrix
...
class Matrix
{
    ...
    public static function validateRow(int $row) : int
    {
        if (!\is_numeric($row) || \intval($row) < 1) {
            throw new \MenuManager\Vendor\Matrix\Exception('Invalid Row');
        }
        return (int) $row;
    }
    /**
    ...
    public static function validateColumn(int $column) : int
    {
        if (!\is_numeric($column) || \intval($column) < 1) {
            throw new \MenuManager\Vendor\Matrix\Exception('Invalid Column');
        }
        return (int) $column;
    }
    /**
    ...
    protected function validateRowInRange(int $row) : int
    {
        ...
        if ($row > $this->rows) {
            throw new \MenuManager\Vendor\Matrix\Exception('Requested Row exceeds matrix size');
        }
        return $row;
    }
    /**
    ...
    protected function validateColumnInRange(int $column) : int
    {
        ...
        if ($column > $this->columns) {
            throw new \MenuManager\Vendor\Matrix\Exception('Requested Column exceeds matrix size');
        }
        return $column;
    }
    /**
     ...
     * @throws Exception
     */
    public function getRows(int $row, int $rowCount = 1) : \MenuManager\Vendor\Matrix\Matrix
    {
     ...
     * @throws Exception
     */
    public function getColumns(int $column, int $columnCount = 1) : \MenuManager\Vendor\Matrix\Matrix
    {
     ...
     * @throws Exception
     */
    public function dropRows(int $row, int $rowCount = 1) : \MenuManager\Vendor\Matrix\Matrix
    {
     ...
     * @throws Exception
     */
    public function dropColumns(int $column, int $columnCount = 1) : \MenuManager\Vendor\Matrix\Matrix
    {
     ...
     */
    public function solve(\MenuManager\Vendor\Matrix\Matrix $B) : \MenuManager\Vendor\Matrix\Matrix
    {
    ...
    public function __get(string $propertyName)
    {
        ...
        throw new \MenuManager\Vendor\Matrix\Exception('Property does not exist');
    }
    ...
    public function __call(string $functionName, $arguments)
    {
        ...
        if (\in_array($functionName, self::$functions, \true)) {
            return \MenuManager\Vendor\Matrix\Functions::$functionName($this, ...$arguments);
        }
        // Test for operation calls
        if (\in_array($functionName, self::$operations, \true)) {
            return \MenuManager\Vendor\Matrix\Operations::$functionName($this, ...$arguments);
        }
        throw new \MenuManager\Vendor\Matrix\Exception('Function or Operation does not exist');
    }
}
