...
use MenuManager\Vendor\Symfony\Component\Config\ConfigCacheFactory;
use MenuManager\Vendor\Symfony\Component\Config\ConfigCacheFactoryInterface;
use MenuManager\Vendor\Symfony\Component\Config\ConfigCacheInterface;
...
use MenuManager\Vendor\Symfony\Component\Translation\Exception\RuntimeException;
use MenuManager\Vendor\Symfony\Component\Translation\Formatter\IntlFormatterInterface;
use MenuManager\Vendor\Symfony\Component\Translation\Formatter\MessageFormatter;
...
use MenuManager\Vendor\Symfony\Contracts\Translation\TranslatableInterface;
use MenuManager\Vendor\Symfony\Contracts\Translation\TranslatorInterface;
// Help opcache.preload discover always-needed symbols
\class_exists(\MenuManager\Vendor\Symfony\Component\Translation\MessageCatalogue::class);
/**
 * @author Fabien Potencier <fabien@symfony.com>
 */
class Translator implements TranslatorInterface, \MenuManager\Vendor\Symfony\Component\Translation\TranslatorBagInterface, LocaleAwareInterface
{
    /**
    ...
    public function trans(?string $id, array $parameters = [], ?string $domain = null, ?string $locale = null) : string
    {
        ...
        $len = \strlen(\MenuManager\Vendor\Symfony\Component\Translation\MessageCatalogue::INTL_DOMAIN_SUFFIX);
        if ($this->hasIntlFormatter && ($catalogue->defines($id, $domain . \MenuManager\Vendor\Symfony\Component\Translation\MessageCatalogue::INTL_DOMAIN_SUFFIX) || \strlen($domain) > $len && 0 === \substr_compare($domain, \MenuManager\Vendor\Symfony\Component\Translation\MessageCatalogue::INTL_DOMAIN_SUFFIX, -$len, $len))) {
...
, $locale, \var_export($this->getAllMessages($this->catalogues[$locale]), \true), $fallbackContent);
    ...
    private function getFallbackContent(\MenuManager\Vendor\Symfony\Component\Translation\MessageCatalogue $catalogue) : string
    {
        $fallbackContent = '';
        $current = '';
...
, $fallbackSuffix, $fallback, \var_export($this->getAllMessages($fallbackCatalogue), \true), $currentSuffix, $fallbackSuffix);
    ...
    private function loadFallbackCatalogues(string $locale) : void
    {
        ...
        foreach ($this->computeFallbackLocales($locale) as $fallback) {
            ...
            $fallbackCatalogue = new \MenuManager\Vendor\Symfony\Component\Translation\MessageCatalogue($fallback, $this->getAllMessages($this->catalogues[$fallback]));
    ...
    private function getAllMessages(\MenuManager\Vendor\Symfony\Component\Translation\MessageCatalogueInterface $catalogue) : array
    {
        $allMessages = [];
        foreach ($catalogue->all() as $domain => $messages) {
            if ($intlMessages = $catalogue->all($domain . \MenuManager\Vendor\Symfony\Component\Translation\MessageCatalogue::INTL_DOMAIN_SUFFIX)) {
                $allMessages[$domain . \MenuManager\Vendor\Symfony\Component\Translation\MessageCatalogue::INTL_DOMAIN_SUFFIX] = $intlMessages;
