...
use ArrayAccess;
use ArrayIterator;
use MenuManager\Vendor\Illuminate\Contracts\Support\CanBeEscapedWhenCastToString;
use MenuManager\Vendor\Illuminate\Support\Traits\EnumeratesValues;
 ...
 */
class Collection implements ArrayAccess, CanBeEscapedWhenCastToString, \MenuManager\Vendor\Illuminate\Support\Enumerable
{
    /**
    ...
    {
        return new \MenuManager\Vendor\Illuminate\Support\LazyCollection($this->items);
    }
    /**
    ...
    {
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::collapse($this->items));
    }
    /**
    ...
    {
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::crossJoin($this->items, ...\array_map([$this, 'getArrayableItems'], $lists)));
    }
    /**
    ...
    public function except($keys)
    {
        ...
        if ($keys instanceof \MenuManager\Vendor\Illuminate\Support\Enumerable) {
            $keys = $keys->all();
        } elseif (!\is_array($keys)) {
            $keys = \func_get_args();
        }
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::except($this->items, $keys));
    }
    /**
    ...
    {
        if ($callback) {
            return new static(\MenuManager\Vendor\Illuminate\Support\Arr::where($this->items, $callback));
        }
    ...
    {
        return \MenuManager\Vendor\Illuminate\Support\Arr::first($this->items, $callback, $default);
    }
    /**
    ...
    {
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::flatten($this->items, $depth));
    }
    /**
    ...
    public function implode($value, $glue = null)
    {
        ...
        $first = $this->first();
        if (\is_array($first) || \is_object($first) && !$first instanceof \MenuManager\Vendor\Illuminate\Support\Stringable) {
    ...
    {
        return \MenuManager\Vendor\Illuminate\Support\Arr::last($this->items, $callback, $default);
    }
    /**
    ...
    {
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::pluck($this->items, $value, $key));
    }
    /**
    ...
    {
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::map($this->items, $callback));
    }
    /**
    ...
    {
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::mapWithKeys($this->items, $callback));
    }
    /**
    ...
    public function only($keys)
    {
        ...
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::only($this->items, $keys));
    }
    /**
    ...
    public function select($keys)
    {
        ...
        $keys = \is_array($keys) ? $keys : \func_get_args();
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::select($this->items, $keys));
    }
    /**
    ...
    {
        $this->items = \MenuManager\Vendor\Illuminate\Support\Arr::prepend($this->items, ...\func_get_args());
        return $this;
    }
    /**
    ...
    {
        return \MenuManager\Vendor\Illuminate\Support\Arr::pull($this->items, $key, $default);
    }
    /**
    ...
    {
        if (\is_null($number)) {
            return \MenuManager\Vendor\Illuminate\Support\Arr::random($this->items);
        }
        if (\is_callable($number)) {
            return new static(\MenuManager\Vendor\Illuminate\Support\Arr::random($this->items, $number($this), $preserveKeys));
        }
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::random($this->items, $number, $preserveKeys));
    }
    /**
    ...
    {
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::shuffle($this->items, $seed));
    }
    /**
    ...
    public function sole($key = null, $operator = null, $value = null)
    {
        ...
        $count = $items->count();
        if ($count === 0) {
            throw new \MenuManager\Vendor\Illuminate\Support\ItemNotFoundException();
        }
        if ($count > 1) {
            throw new \MenuManager\Vendor\Illuminate\Support\MultipleItemsFoundException($count);
        }
        return $items->first();
    }
    /**
    ...
    protected function sortByMany(array $comparisons = [], int $options = \SORT_REGULAR)
    {
        ...
        \uasort($items, function ($a, $b) use($comparisons, $options) {
            foreach ($comparisons as $comparison) {
                $comparison = \MenuManager\Vendor\Illuminate\Support\Arr::wrap($comparison);
                $prop = $comparison[0];
                $ascending = \MenuManager\Vendor\Illuminate\Support\Arr::get($comparison, 1, \true) === \true || \MenuManager\Vendor\Illuminate\Support\Arr::get($comparison, 1, \true) === 'asc';
    ...
    public function sortByDesc($callback, $options = \SORT_REGULAR)
    {
        if (\is_array($callback) && !\is_callable($callback)) {
            foreach ($callback as $index => $key) {
                $comparison = \MenuManager\Vendor\Illuminate\Support\Arr::wrap($key);
                $comparison[1] = 'desc';
    ...
    public function dot()
    {
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::dot($this->all()));
    }
    /**
    ...
    {
        return new static(\MenuManager\Vendor\Illuminate\Support\Arr::undot($this->all()));
    }
    /**
