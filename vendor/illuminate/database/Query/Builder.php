...
use MenuManager\Vendor\Illuminate\Contracts\Database\Query\Builder as BuilderContract;
use MenuManager\Vendor\Illuminate\Contracts\Database\Query\ConditionExpression;
...
use MenuManager\Vendor\Illuminate\Database\Concerns\BuildsQueries;
use MenuManager\Vendor\Illuminate\Database\Concerns\ExplainsQueries;
use MenuManager\Vendor\Illuminate\Database\ConnectionInterface;
...
use MenuManager\Vendor\Illuminate\Database\Query\Grammars\Grammar;
use MenuManager\Vendor\Illuminate\Database\Query\Processors\Processor;
use MenuManager\Vendor\Illuminate\Pagination\Paginator;
use MenuManager\Vendor\Illuminate\Support\Arr;
use MenuManager\Vendor\Illuminate\Support\Collection;
use MenuManager\Vendor\Illuminate\Support\LazyCollection;
...
class Builder implements BuilderContract
{
    ...
    {
        $this->addSelect(new \MenuManager\Vendor\Illuminate\Database\Query\Expression($expression));
        if ($bindings) {
    ...
    {
        $this->from = new \MenuManager\Vendor\Illuminate\Database\Query\Expression($expression);
    ...
    {
        $this->indexHint = new \MenuManager\Vendor\Illuminate\Database\Query\IndexHint('hint', $index);
        return $this;
    }
    /**
    ...
    {
        $this->indexHint = new \MenuManager\Vendor\Illuminate\Database\Query\IndexHint('force', $index);
        return $this;
    }
    /**
    ...
    {
        $this->indexHint = new \MenuManager\Vendor\Illuminate\Database\Query\IndexHint('ignore', $index);
        return $this;
    }
    /**
    ...
    public function joinSub($query, $as, $first, $operator = null, $second = null, $type = 'inner', $where = \false)
    {
        ...
        return $this->join(new \MenuManager\Vendor\Illuminate\Database\Query\Expression($expression), $first, $operator, $second, $type, $where);
    }
    /**
    ...
    public function joinLateral($query, string $as, string $type = 'inner')
    {
        ...
        $expression = '(' . $query . ') as ' . $this->grammar->wrapTable($as);
        $this->addBinding($bindings, 'join');
        $this->joins[] = $this->newJoinLateralClause($this, $type, new \MenuManager\Vendor\Illuminate\Database\Query\Expression($expression));
        return $this;
    }
    /**
    ...
    public function crossJoinSub($query, $as)
    {
        ...
        $expression = '(' . $query . ') as ' . $this->grammar->wrapTable($as);
        $this->addBinding($bindings, 'join');
        $this->joins[] = $this->newJoinClause($this, 'cross', new \MenuManager\Vendor\Illuminate\Database\Query\Expression($expression));
        return $this;
    }
    /**
    ...
    {
        return new \MenuManager\Vendor\Illuminate\Database\Query\JoinClause($parentQuery, $type, $table);
    }
    /**
    ...
    {
        return new \MenuManager\Vendor\Illuminate\Database\Query\JoinLateralClause($parentQuery, $type, $table);
    }
    /**
    ...
    public function where($column, $operator = null, $value = null, $boolean = 'and')
    {
        ...
        if ($this->isQueryable($column) && !\is_null($operator)) {
            ...
            return $this->addBinding($bindings, 'where')->where(new \MenuManager\Vendor\Illuminate\Database\Query\Expression('(' . $sub . ')'), $operator, $value, $boolean);
        }
        ...
        if (\str_contains($columnString, '->') && \is_bool($value)) {
            $value = new \MenuManager\Vendor\Illuminate\Database\Query\Expression($value ? 'true' : 'false');
            if (\is_string($column)) {
                $type = 'JsonBoolean';
            }
        }
    ...
    public function whereIn($column, $values, $boolean = 'and', $not = \false)
    {
        ...
        if ($this->isQueryable($values)) {
            ...
            $values = [new \MenuManager\Vendor\Illuminate\Database\Query\Expression($query)];
    ...
    {
        $this->groups[] = new \MenuManager\Vendor\Illuminate\Database\Query\Expression($sql);
    ...
    public function orderBy($column, $direction = 'asc')
    {
        if ($this->isQueryable($column)) {
            ...
            $column = new \MenuManager\Vendor\Illuminate\Database\Query\Expression('(' . $query . ')');
    ...
    protected function runPaginationCountQuery($columns = ['*'])
    {
        if ($this->groups || $this->havings) {
            ...
            return $this->newQuery()->from(new \MenuManager\Vendor\Illuminate\Database\Query\Expression('(' . $clone->toSql() . ') as ' . $this->grammar->wrap('aggregate_table')))->mergeBindings($clone)->setAggregate('count', $this->withoutSelectAliases($columns))->get()->all();
        }
