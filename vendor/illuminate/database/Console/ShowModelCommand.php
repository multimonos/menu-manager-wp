...
use BackedEnum;
use MenuManager\Vendor\Doctrine\DBAL\Schema\Column;
use MenuManager\Vendor\Doctrine\DBAL\Schema\Index;
use MenuManager\Vendor\Doctrine\DBAL\Types\DecimalType;
...
use MenuManager\Vendor\Illuminate\Support\Facades\Gate;
...
class ShowModelCommand extends \MenuManager\Vendor\Illuminate\Database\Console\DatabaseInspectionCommand
{
    /**
    ...
    protected function displayCli($class, $database, $table, $policy, $attributes, $relations, $observers)
    {
        ...
        foreach ($attributes as $attribute) {
            $first = \trim(\sprintf('%s %s', $attribute['name'], collect(['increments', 'unique', 'nullable', 'fillable', 'hidden', 'appended'])->filter(fn($property) => $attribute[$property])->map(fn($property) => \sprintf('<fg=gray>%s</>', $property))->implode('<fg=gray>,</> ')));
