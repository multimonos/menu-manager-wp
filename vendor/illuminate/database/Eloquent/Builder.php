...
use Closure;
use Exception;
use MenuManager\Vendor\Illuminate\Contracts\Database\Eloquent\Builder as BuilderContract;
use MenuManager\Vendor\Illuminate\Contracts\Database\Query\Expression;
...
use MenuManager\Vendor\Illuminate\Database\Eloquent\Concerns\QueriesRelationships;
...
use MenuManager\Vendor\Illuminate\Database\RecordsNotFoundException;
use MenuManager\Vendor\Illuminate\Database\UniqueConstraintViolationException;
...
class Builder implements BuilderContract
{
    ...
    {
        if ($id instanceof \MenuManager\Vendor\Illuminate\Database\Eloquent\Model) {
            $id = $id->getKey();
        }
    ...
    public function hydrate(array $items)
    {
        ...
        return $instance->newCollection(\array_map(function ($item) use($items, $instance) {
            ...
            if (\count($items) > 1) {
                $model->preventsLazyLoading = \MenuManager\Vendor\Illuminate\Database\Eloquent\Model::preventsLazyLoading();
            }
            return $model;
        }, $items));
    }
    ...
    public function findOrFail($id, $columns = ['*'])
    {
        ...
        if (\is_array($id)) {
            if (\count($result) !== \count(\array_unique($id))) {
                throw (new \MenuManager\Vendor\Illuminate\Database\Eloquent\ModelNotFoundException())->setModel(\get_class($this->model), \array_diff($id, $result->modelKeys()));
            }
            return $result;
        }
        if (\is_null($result)) {
            throw (new \MenuManager\Vendor\Illuminate\Database\Eloquent\ModelNotFoundException())->setModel(\get_class($this->model), $id);
        }
        return $result;
    }
    /**
    ...
    public function firstOrFail($columns = ['*'])
    {
        ...
        throw (new \MenuManager\Vendor\Illuminate\Database\Eloquent\ModelNotFoundException())->setModel(\get_class($this->model));
    }
    /**
    ...
    public function getRelation($name)
    {
        ...
        $relation = Relation::noConstraints(function () use($name) {
            try {
                ...
                throw \MenuManager\Vendor\Illuminate\Database\Eloquent\RelationNotFoundException::make($this->getModel(), $name);
            }
        });
    ...
    {
        return \MenuManager\Vendor\Illuminate\Database\Eloquent\Model::withoutEvents(fn() => $this->forceCreate($attributes));
    }
    /**
    ...
    public function applyScopes()
    {
        ...
        foreach ($this->scopes as $identifier => $scope) {
            ...
            $builder->callScope(function (self $builder) use($scope) {
                ...
                if ($scope instanceof \MenuManager\Vendor\Illuminate\Database\Eloquent\Scope) {
     ...
     * @return $this
     */
    public function setModel(\MenuManager\Vendor\Illuminate\Database\Eloquent\Model $model)
    {
        $this->model = $model;
    ...
    public function __get($key)
    {
        if (\in_array($key, ['orWhere', 'whereNot', 'orWhereNot'])) {
            return new \MenuManager\Vendor\Illuminate\Database\Eloquent\HigherOrderBuilderProxy($this, $key);
        }
