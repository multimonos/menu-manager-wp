...
use MenuManager\Vendor\Doctrine\DBAL\Connection as DoctrineConnection;
use MenuManager\Vendor\Doctrine\DBAL\Types\Type;
use Exception;
use MenuManager\Vendor\Illuminate\Contracts\Events\Dispatcher;
use MenuManager\Vendor\Illuminate\Database\Events\QueryExecuted;
use MenuManager\Vendor\Illuminate\Database\Events\StatementPrepared;
use MenuManager\Vendor\Illuminate\Database\Events\TransactionBeginning;
use MenuManager\Vendor\Illuminate\Database\Events\TransactionCommitted;
use MenuManager\Vendor\Illuminate\Database\Events\TransactionCommitting;
use MenuManager\Vendor\Illuminate\Database\Events\TransactionRolledBack;
use MenuManager\Vendor\Illuminate\Database\Query\Builder as QueryBuilder;
use MenuManager\Vendor\Illuminate\Database\Query\Expression;
use MenuManager\Vendor\Illuminate\Database\Query\Grammars\Grammar as QueryGrammar;
use MenuManager\Vendor\Illuminate\Database\Query\Processors\Processor;
use MenuManager\Vendor\Illuminate\Database\Schema\Builder as SchemaBuilder;
use MenuManager\Vendor\Illuminate\Support\Arr;
use MenuManager\Vendor\Illuminate\Support\InteractsWithTime;
use MenuManager\Vendor\Illuminate\Support\Traits\Macroable;
use PDO;
use PDOStatement;
use RuntimeException;
class Connection implements \MenuManager\Vendor\Illuminate\Database\ConnectionInterface
{
    use \MenuManager\Vendor\Illuminate\Database\DetectsConcurrencyErrors, \MenuManager\Vendor\Illuminate\Database\DetectsLostConnections, \MenuManager\Vendor\Illuminate\Database\Concerns\ManagesTransactions, InteractsWithTime, Macroable;
    /**
    ...
    public function scalar($query, $bindings = [], $useReadPdo = \true)
    {
        ...
        $record = (array) $record;
        if (\count($record) > 1) {
            throw new \MenuManager\Vendor\Illuminate\Database\MultipleColumnsSelectedException();
        }
        return \reset($record);
    }
    /**
    ...
    protected function run($query, $bindings, Closure $callback)
    {
        ...
        } catch (\MenuManager\Vendor\Illuminate\Database\QueryException $e) {
    ...
    protected function runQueryCallback($query, $bindings, Closure $callback)
    {
        ...
        try {
            ...
            if ($this->isUniqueConstraintError($e)) {
                throw new \MenuManager\Vendor\Illuminate\Database\UniqueConstraintViolationException($this->getName(), $query, $this->prepareBindings($bindings), $e);
            }
            throw new \MenuManager\Vendor\Illuminate\Database\QueryException($this->getName(), $query, $this->prepareBindings($bindings), $e);
        }
    }
    /**
     ...
     * @throws \Illuminate\Database\QueryException
     */
    protected function handleQueryException(\MenuManager\Vendor\Illuminate\Database\QueryException $e, $query, $bindings, Closure $callback)
    {
     ...
     * @throws \Illuminate\Database\QueryException
     */
    protected function tryAgainIfCausedByLostConnection(\MenuManager\Vendor\Illuminate\Database\QueryException $e, $query, $bindings, Closure $callback)
    {
    ...
    public function reconnect()
    {
        ...
        throw new \MenuManager\Vendor\Illuminate\Database\LostConnectionException('Lost connection and no reconnector available.');
    }
    /**
    ...
    {
        $this->events?->listen(\MenuManager\Vendor\Illuminate\Database\Events\QueryExecuted::class, $callback);
    }
    /**
    ...
    {
        return \class_exists('MenuManager\\Vendor\\Doctrine\\DBAL\\Connection');
    }
    /**
     ...
     * @return $this
     */
    public function setQueryGrammar(\MenuManager\Vendor\Illuminate\Database\Query\Grammars\Grammar $grammar)
    {
     ...
     * @return $this
     */
    public function setSchemaGrammar(\MenuManager\Vendor\Illuminate\Database\Schema\Grammars\Grammar $grammar)
    {
     ...
     */
    public function withTablePrefix(\MenuManager\Vendor\Illuminate\Database\Grammar $grammar)
    {
