...
class Benchmark
{
    ...
    {
        return collect(\MenuManager\Vendor\Illuminate\Support\Arr::wrap($benchmarkables))->map(function ($callback) use($iterations) {
    ...
    {
        $result = collect(static::measure(\MenuManager\Vendor\Illuminate\Support\Arr::wrap($benchmarkables), $iterations))->map(fn($average) => \number_format($average, 3) . 'ms')->when($benchmarkables instanceof Closure, fn($c) => $c->first(), fn($c) => $c->all());
        dd($result);
    }
}
