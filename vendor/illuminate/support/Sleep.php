...
class Sleep
{
    ...
    public static function until($timestamp)
    {
        if (\is_numeric($timestamp)) {
            $timestamp = \MenuManager\Vendor\Illuminate\Support\Carbon::createFromTimestamp($timestamp);
        }
        return new static(\MenuManager\Vendor\Illuminate\Support\Carbon::now()->diff($timestamp));
    }
    /**
    ...
    public function __destruct()
    {
        ...
        if (static::$fake) {
            ...
            if (static::$syncWithCarbon) {
                \MenuManager\Vendor\Illuminate\Support\Carbon::setTestNow(\MenuManager\Vendor\Illuminate\Support\Carbon::now()->add($this->duration));
            }
    ...
    public static function assertSequence($sequence)
    {
        ...
        collect($sequence)->zip(static::$sequence)->eachSpread(function (?\MenuManager\Vendor\Illuminate\Support\Sleep $expected, CarbonInterval $actual) {
            if ($expected === null) {
                return;
            }
