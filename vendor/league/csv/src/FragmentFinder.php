...
class FragmentFinder
{
     ...
     */
    public function findAll(string $expression, \MenuManager\Vendor\League\Csv\TabularData $tabularData) : iterable
    {
     ...
     * @throws SyntaxError
     */
    public function findFirst(string $expression, \MenuManager\Vendor\League\Csv\TabularData $tabularData) : ?\MenuManager\Vendor\League\Csv\TabularDataReader
    {
     ...
     */
    public function findFirstOrFail(string $expression, \MenuManager\Vendor\League\Csv\TabularData $tabularData) : \MenuManager\Vendor\League\Csv\TabularDataReader
    {
        ...
        if ([] !== array_filter($parsedExpression['selections'], fn(array $selection) => -1 === $selection['start'])) {
            throw new \MenuManager\Vendor\League\Csv\FragmentNotFound('The expression `' . $expression . '` contains an invalid or an unsupported selection for the tabular data.');
        }
        $fragment = $this->find($parsedExpression, $tabularData)[0];
        return match ([]) {
            $fragment->first() => throw new \MenuManager\Vendor\League\Csv\FragmentNotFound('No fragment found in the tabular data with the expression `' . $expression . '`.'),
            default => $fragment,
        };
    }
    /**
     ...
     */
    private function find(array $parsedExpression, \MenuManager\Vendor\League\Csv\TabularData $tabularData) : array
    {
        ['type' => $type, 'selections' => $selections] = $parsedExpression;
        $selections = array_filter($selections, fn(array $selection) => -1 !== $selection['start']);
        if ([] === $selections) {
            return [new \MenuManager\Vendor\League\Csv\ResultSet()];
        }
        if (self::TYPE_ROW === $type) {
            ...
            return [(new \MenuManager\Vendor\League\Csv\Statement())->where($rowFilter)->process($tabularData)];
        }
        if (self::TYPE_COLUMN === $type) {
            $columns = array_reduce($selections, fn(array $columns, array $selection) => [...$columns, ...$selection['columns']], []);
            return [match ([]) {
                $columns => new \MenuManager\Vendor\League\Csv\ResultSet(),
                default => (new \MenuManager\Vendor\League\Csv\Statement())->select(...$columns)->process($tabularData),
            }];
        }
        return array_map(fn(array $selection) => (new \MenuManager\Vendor\League\Csv\Statement())->offset($selection['start'])->limit($selection['length'])->select(...$selection['columns'])->process($tabularData), $selections);
    }
    /**
     * @return array{type:string, selections:non-empty-array<selection>}
     */
    private function parseExpression(string $expression, \MenuManager\Vendor\League\Csv\TabularData $tabularData) : array
    {
     ...
     * @return selection
     */
    private function parseColumnSelection(string $selection, \MenuManager\Vendor\League\Csv\TabularData $tabularData) : array
    {
        [$start, $end] = $this->parseRowColumnSelection($selection);
     ...
     * @return selection
     */
    private function parseCellSelection(string $selection, \MenuManager\Vendor\League\Csv\TabularData $tabularData) : array
    {
