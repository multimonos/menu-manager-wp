...
use Closure;
use Iterator;
use MenuManager\Vendor\League\Csv\Query\Constraint\Criteria;
...
final class Buffer implements \MenuManager\Vendor\League\Csv\TabularData
{
    ...
    public function __construct(array $header = [])
    {
        $this->header = match (\true) {
            !array_is_list($header) => throw new \MenuManager\Vendor\League\Csv\SyntaxError('The header must be a list of unique column names.'),
     ...
     */
    public static function from(PDOStatement|Result|mysqli_result|SQLite3Result|\MenuManager\Vendor\League\Csv\TabularData $dataStorage, int $options = self::INCLUDE_HEADER) : self
    {
        /** @var Iterator<int, array> $rows */
        $rows = $dataStorage instanceof \MenuManager\Vendor\League\Csv\TabularData ? $dataStorage->getRecords() : \MenuManager\Vendor\League\Csv\RdbmsResult::rows($dataStorage);
        $instance = new self(match (\true) {
            self::EXCLUDE_HEADER === $options => [],
            $dataStorage instanceof \MenuManager\Vendor\League\Csv\TabularData => $dataStorage->getHeader(),
            default => \MenuManager\Vendor\League\Csv\RdbmsResult::columnNames($dataStorage),
        });
        /**
         * @var int $offset
         * @var list<mixed> $row
         */
        foreach (new \MenuManager\Vendor\League\Csv\MapIterator($rows, fn(array $record): array => array_values($record)) as $offset => $row) {
     ...
     * @throws Exception
     */
    public function to(\MenuManager\Vendor\League\Csv\TabularDataWriter $dataStorage, int $options = self::INCLUDE_HEADER) : int
    {
        $bytes = 0;
    ...
    public function getRecords(array $header = []) : Iterator
    {
        $header = match (\true) {
            [] === $header => $this->header,
            $header !== array_filter($header, is_int(...), ARRAY_FILTER_USE_KEY) => throw new \MenuManager\Vendor\League\Csv\SyntaxError('The header must be a list of unique column names.'),
            $header !== array_filter($header, \is_string(...)) => throw \MenuManager\Vendor\League\Csv\SyntaxError::dueToInvalidHeaderColumnNames(),
            $header !== array_unique($header) => throw \MenuManager\Vendor\League\Csv\SyntaxError::dueToDuplicateHeaderColumnNames($header),
            default => $header,
        };
        return \MenuManager\Vendor\League\Csv\MapIterator::fromIterable($this->rows, fn(array $row): array => $this->rowToRecord($row, $header));
    }
    /**
    ...
    {
        return \MenuManager\Vendor\League\Csv\MapIterator::fromIterable($this->getRecords(), $callback);
    }
    /**
    ...
    {
        -1 < $nth || throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidRecordOffset($nth, $method);
    ...
    {
        if (is_int($index)) {
            $index > -1 || throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidColumnIndex($index, 'offset', __METHOD__);
            [] === $this->header || array_key_exists($index, $this->header) || throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidColumnIndex($index, 'name', __METHOD__);
            $iterator = new \MenuManager\Vendor\League\Csv\MapIterator($this->getRecords(), fn(array $row) => array_values($row));
            $iterator = new CallbackFilterIterator($iterator, fn(array $row) => array_key_exists($index, $row));
            return new \MenuManager\Vendor\League\Csv\MapIterator($iterator, fn(array $row) => $row[$index]);
        }
        [] !== $this->header || throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidColumnIndex($index, 'name', __METHOD__);
        in_array($index, $this->header, \true) || throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidColumnIndex($index, 'name', __METHOD__);
    ...
    {
        [] !== $records || throw \MenuManager\Vendor\League\Csv\CannotInsertRecord::triggerOnValidation('@buffer_record_validation_on_insert', $records);
    ...
    public function update(Predicate|Closure|callable $where, array $record) : int
    {
        ...
        $iterator = new \MenuManager\Vendor\League\Csv\MapIterator(new CallbackFilterIterator($this->getRecords(), $this->filterPredicate($where)), $updateRecord);
    ...
    {
        $this->filterInsertRecord($record) || throw \MenuManager\Vendor\League\Csv\CannotInsertRecord::triggerOnValidation('@buffer_record_validation_on_insert', $record);
    ...
    {
        [] !== $record || throw \MenuManager\Vendor\League\Csv\CannotInsertRecord::triggerOnValidation('@buffer_record_validation_on_update', $record);
        if (array_is_list($record)) {
        ...
        return match (\true) {
            ...
            $keys !== array_filter($keys, \is_string(...)), [] !== array_diff($keys, $this->header) => throw \MenuManager\Vendor\League\Csv\CannotInsertRecord::triggerOnValidation('@buffer_record_validation_on_update', $record),
            default => $record,
        };
    }
    /**
    ...
    private function validateRecord(array $record) : array
    {
        ...
        foreach ($this->validators as $name => $validator) {
            \true === $validator($record) || throw \MenuManager\Vendor\League\Csv\CannotInsertRecord::triggerOnValidation($name, $record);
        }
