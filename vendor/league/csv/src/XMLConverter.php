...
use MenuManager\Vendor\Dom\Element;
use MenuManager\Vendor\Dom\XMLDocument;
use DOMDocument;
use DOMElement;
use DOMException;
use Exception;
...
class XMLConverter
{
    ...
    public function download(iterable $records, ?string $filename = null, string $encoding = 'utf-8', bool $formatOutput = \false) : int|false
    {
        ...
        if (null !== $filename) {
            \MenuManager\Vendor\League\Csv\HttpHeaders::forFileDownload($filename, 'application/xml; charset=' . strtolower($encoding));
        }
    ...
    public function import(iterable $records, DOMDocument|XMLDocument $doc) : DOMElement|Element
    {
        if (null !== $this->formatter) {
            $records = \MenuManager\Vendor\League\Csv\MapIterator::fromIterable($records, $this->formatter);
        }
