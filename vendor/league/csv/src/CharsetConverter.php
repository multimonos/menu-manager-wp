...
class CharsetConverter extends php_user_filter
{
     ...
     */
    public static function addTo(\MenuManager\Vendor\League\Csv\AbstractCsv $csv, string $input_encoding, string $output_encoding, ?array $params = null) : \MenuManager\Vendor\League\Csv\AbstractCsv
    {
        self::register();
        if ($csv instanceof \MenuManager\Vendor\League\Csv\Reader) {
    ...
    public function filter($in, $out, &$consumed, bool $closing) : int
    {
        ...
        if ($this->skipBomSequence && null !== ($bom = \MenuManager\Vendor\League\Csv\Bom::tryFromSequence($inputBuffer))) {
    ...
    public function convert(iterable $records) : iterable
    {
        return match (\true) {
            ...
            default => \MenuManager\Vendor\League\Csv\MapIterator::fromIterable($records, $this),
        };
    }
    /**
     ...
     */
    public static function addBOMSkippingTo(\MenuManager\Vendor\League\Csv\Reader $document, string $output_encoding = 'UTF-8') : \MenuManager\Vendor\League\Csv\Reader
    {
        self::register();
        $document->appendStreamFilterOnRead(self::getFiltername((\MenuManager\Vendor\League\Csv\Bom::tryFrom($document->getInputBOM()) ?? \MenuManager\Vendor\League\Csv\Bom::Utf8)->encoding(), $output_encoding), [self::BOM_SEQUENCE => self::SKIP_BOM_SEQUENCE]);
        return $document;
    }
}
