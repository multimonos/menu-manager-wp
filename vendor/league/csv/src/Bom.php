...
enum Bom : string
{
    ...
    {
        $str = match (\true) {
            $sequence instanceof SplFileObject, $sequence instanceof \MenuManager\Vendor\League\Csv\Stream => self::getContents($sequence, 4, 0),
            \is_resource($sequence) => \stream_get_contents($sequence, 4, 0),
            $sequence instanceof \MenuManager\Vendor\League\Csv\AbstractCsv => $sequence->getInputBOM(),
    ...
    private static function getContents(\MenuManager\Vendor\League\Csv\Stream|SplFileObject $sequence, int $length, int $offset) : ?string
    {
