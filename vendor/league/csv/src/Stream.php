...
final class Stream implements SeekableIterator
{
    ...
    {
        throw \MenuManager\Vendor\League\Csv\UnavailableStream::dueToForbiddenCloning(self::class);
    }
    ...
    public static function createFromPath(string $path, string $open_mode = 'r', $context = null) : self
    {
        ...
        $resource = fopen(...$args);
        restore_error_handler();
        is_resource($resource) || throw \MenuManager\Vendor\League\Csv\UnavailableStream::dueToPathNotFound($path);
    ...
    public function appendFilter(string $filtername, int $read_write, mixed $params = null) : void
    {
        ...
        restore_error_handler();
        is_resource($res) || throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToStreamFilterNotFound($filtername);
    ...
    {
        return match (\true) {
            1 !== strlen($delimiter) => throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidDelimiterCharacter($delimiter, $caller),
            1 !== strlen($enclosure) => throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidEnclosureCharacter($enclosure, $caller),
            1 !== strlen($escape) && '' !== $escape => throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidEscapeCharacter($escape, $caller),
    ...
    {
        $this->is_seekable || throw \MenuManager\Vendor\League\Csv\UnavailableFeature::dueToMissingStreamSeekability();
    ...
    {
        $offset >= 0 || throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidSeekingPosition($offset, __METHOD__);
        $this->rewind();
