 ...
 */
class Writer extends \MenuManager\Vendor\League\Csv\AbstractCsv implements \MenuManager\Vendor\League\Csv\TabularDataWriter
{
    ...
    protected function resetProperties() : void
    {
        ...
        $this->insertRecord = match ($this->enclose_all) {
            self::ENCLOSE_ALL => fn(array $record): int|false => $this->document->fwrite(implode($this->delimiter, array_map(fn($content) => $this->enclosure . $content . $this->enclosure, str_replace($this->enclosure_replace[0], $this->enclosure_replace[1], $record))) . $this->newline),
    ...
    public function insertOne(array $record) : int
    {
        ...
        if (\false === $bytes) {
            throw \MenuManager\Vendor\League\Csv\CannotInsertRecord::triggerOnInsertion($record);
        }
    ...
    public function setFlushThreshold(?int $threshold) : self
    {
        ...
        null === $threshold || 1 <= $threshold || throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidThreshold($threshold, __METHOD__);
