 ...
 */
class Reader extends \MenuManager\Vendor\League\Csv\AbstractCsv implements \MenuManager\Vendor\League\Csv\TabularDataReader, JsonSerializable
{
    ...
    public function setHeaderOffset(?int $offset) : static
    {
        ...
        null === $offset || -1 < $offset || throw \MenuManager\Vendor\League\Csv\InvalidArgument::dueToInvalidHeaderOffset($offset, __METHOD__);
    ...
    protected function setHeader(int $offset) : array
    {
        ...
        if (0 === $offset) {
            $inputBom = \MenuManager\Vendor\League\Csv\Bom::tryFrom($this->getInputBOM());
            $header = $this->removeBOM($header, !$this->is_input_bom_included ? $inputBom?->length() ?? 0 : 0, $this->enclosure);
        }
        return match (\true) {
            [] === $header, [null] === $header, [\false] === $header, [''] === $header && 0 === $offset && null !== $inputBom => throw \MenuManager\Vendor\League\Csv\SyntaxError::dueToHeaderNotFound($offset),
            default => $header,
        };
    }
    /**
     ...
     */
    protected function getDocument() : SplFileObject|\MenuManager\Vendor\League\Csv\Stream
    {
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->fetchColumn($index);
    }
    public function value(int|string $column = 0) : mixed
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->value($column);
    }
    /**
     * @throws Exception
     */
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->first();
    }
    /**
     * @throws Exception
     */
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->nth($nth);
    }
    /**
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->nthAsObject($nth, $className, $header);
    }
    /**
     * @param class-string $className
     *
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->firstAsObject($className, $header);
    }
    public function fetchPairs(string|int $offset_index = 0, string|int $value_index = 1) : Iterator
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->fetchPairs($offset_index, $value_index);
    }
    /**
     * @throws Exception
     */
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->each($callback);
    }
    /**
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->exists($callback);
    }
    /**
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->reduce($callback, $initial);
    }
    /**
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->chunkBy($recordsCount);
    }
    /**
     ...
     * @throws SyntaxError
     */
    public function slice(int $offset, int $length = -1) : \MenuManager\Vendor\League\Csv\TabularDataReader
    {
        return (new \MenuManager\Vendor\League\Csv\Statement())->offset($offset)->limit($length)->process($this);
    }
    /**
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->select(...$columns);
    }
    public function selectAllExcept(string|int ...$columns) : \MenuManager\Vendor\League\Csv\TabularDataReader
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->selectAllExcept(...$columns);
    }
    /**
    ...
    protected function prepareRecords() : Iterator
    {
        ...
        if (!$this->is_input_bom_included) {
            $bom = \MenuManager\Vendor\League\Csv\Bom::tryFrom($this->getInputBOM());
        }
        ...
        if ($this->is_empty_records_included) {
            $records = new \MenuManager\Vendor\League\Csv\MapIterator($records, fn(array $record): array => [null] === $record ? [] : $record);
        }
        return $records;
    }
    /**
     ...
     */
    protected function stripBOM(Iterator $iterator, ?\MenuManager\Vendor\League\Csv\Bom $bom) : Iterator
    {
        if (null === $bom) {
            return $iterator;
        }
        ...
        return new CallbackFilterIterator(new \MenuManager\Vendor\League\Csv\MapIterator($iterator, $mapper), fn(array $record): bool => $this->is_empty_records_included || $record !== [null]);
    }
    /**
     * @param array<string> $header
     *
    ...
    protected function combineHeader(Iterator $iterator, array $header) : Iterator
    {
        ...
        return match ([]) {
            $header => new \MenuManager\Vendor\League\Csv\MapIterator($iterator, $formatter(...)),
            default => new \MenuManager\Vendor\League\Csv\MapIterator($iterator, function (array $record) use($header, $formatter) : array {
                $assocRecord = [];
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->fetchColumnByName($name);
    }
    /**
     * DEPRECATION WARNING! This method will be removed in the next major point release.
     *
    ...
    {
        return \MenuManager\Vendor\League\Csv\ResultSet::from($this)->fetchColumnByOffset($offset);
    }
    /**
     * DEPRECATION WARNING! This method will be removed in the next major point release.
     *
