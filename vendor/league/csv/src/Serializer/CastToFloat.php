 ...
 */
final class CastToFloat implements \MenuManager\Vendor\League\Csv\Serializer\TypeCasting
{
    ...
    public function toVariable(mixed $value) : ?float
    {
        if (null === $value) {
            return match ($this->isNullable) {
                \true => $this->default,
                \false => throw \MenuManager\Vendor\League\Csv\Serializer\TypeCastingFailed::dueToNotNullableType('float', info: $this->info),
            };
        }
        ...
        $float = filter_var($value, \MenuManager\Vendor\League\Csv\Serializer\Type::Float->filterFlag());
        return match ($float) {
            \false => throw \MenuManager\Vendor\League\Csv\Serializer\TypeCastingFailed::dueToInvalidValue($value, \MenuManager\Vendor\League\Csv\Serializer\Type::Float->value, info: $this->info),
            default => $float,
        };
    }
    private function init(ReflectionProperty|ReflectionParameter $reflectionProperty) : bool
    {
        ...
        foreach (\MenuManager\Vendor\League\Csv\Serializer\Type::list($reflectionProperty) as $found) {
            ...
            if (null === $type && $found[0]->isOneOf(\MenuManager\Vendor\League\Csv\Serializer\Type::Mixed, \MenuManager\Vendor\League\Csv\Serializer\Type::Float)) {
                $type = $found;
            }
        }
        null !== $type || throw throw \MenuManager\Vendor\League\Csv\Serializer\MappingFailed::dueToTypeCastingUnsupportedType($reflectionProperty, $this, 'float', 'null', 'mixed');
        return $isNullable;
    }
}
