...
final class MapRecord
{
    ...
    public function afterMappingMethods(ReflectionClass $class) : array
    {
        ...
        foreach ($this->afterMapping as $method) {
            try {
                ...
                throw new \MenuManager\Vendor\League\Csv\Serializer\MappingFailed('The method `' . $method . '` is not defined on the `' . $class->getName() . '` class.', 0, $exception);
            }
            0 === $accessor->getNumberOfRequiredParameters() || throw new \MenuManager\Vendor\League\Csv\Serializer\MappingFailed('The method `' . $class->getName() . '::' . $accessor->getName() . '` has too many required parameters.');
            $methods[] = $accessor;
        }
        return $methods;
    }
    ...
    public static function tryFrom(ReflectionClass $class) : ?self
    {
        ...
        return match ($nbAttributes) {
            ...
            default => throw new \MenuManager\Vendor\League\Csv\Serializer\MappingFailed('Using more than one `' . self::class . '` attribute on a class property or method is not supported.'),
        };
    }
}
