...
{
    public function __construct(public readonly int|string $source, public readonly \MenuManager\Vendor\League\Csv\Serializer\TypeCastingTargetType $targetType, public readonly string $targetName, public readonly ?string $targetMethodName, public readonly ?string $targetClassName)
    {
    }
    ...
    public static function fromMethodFirstArgument(ReflectionParameter $accessor) : self
    {
        ...
        return new self(self::resolveSource($method), \MenuManager\Vendor\League\Csv\Serializer\TypeCastingTargetType::MethodFirstArgument, $accessor->getName(), $method->getName(), $className);
    }
    public static function fromProperty(ReflectionProperty $accessor) : self
    {
        ...
        return new self($source, \MenuManager\Vendor\League\Csv\Serializer\TypeCastingTargetType::PropertyName, $accessor->getName(), null, $className);
    }
    private static function resolveSource(ReflectionFunctionAbstract $method) : int|string
    {
        $attributes = $method->getAttributes(\MenuManager\Vendor\League\Csv\Serializer\MapCell::class, ReflectionAttribute::IS_INSTANCEOF);
        if ([] === $attributes) {
