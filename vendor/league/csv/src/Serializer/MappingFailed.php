...
use Throwable;
final class MappingFailed extends LogicException implements \MenuManager\Vendor\League\Csv\Serializer\SerializationFailed
{
    public static function dueToUnsupportedType(ReflectionProperty|ReflectionParameter $reflectionProperty) : self
    {
        $suffix = 'is missing; register it using the `' . \MenuManager\Vendor\League\Csv\Serializer\Denormalizer::class . '` class.';
    ...
    public static function dueToTypeCastingUnsupportedType(ReflectionProperty|ReflectionParameter $reflectionProperty, \MenuManager\Vendor\League\Csv\Serializer\TypeCasting $typeCasting, string ...$types) : self
    {
    ...
    {
        return new self('`' . $typeCaster . '` must be an resolvable class implementing the `' . \MenuManager\Vendor\League\Csv\Serializer\TypeCasting::class . '` interface or a supported alias.');
    }
}
