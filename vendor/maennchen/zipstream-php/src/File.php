...
use MenuManager\Vendor\ZipStream\Exception\FileSizeIncorrectException;
use MenuManager\Vendor\ZipStream\Exception\OverflowException;
use MenuManager\Vendor\ZipStream\Exception\ResourceActionException;
use MenuManager\Vendor\ZipStream\Exception\SimulationFileUnknownException;
use MenuManager\Vendor\ZipStream\Exception\StreamNotReadableException;
use MenuManager\Vendor\ZipStream\Exception\StreamNotSeekableException;
/**
 * @internal
 */
class File
{
    ...
    private \MenuManager\Vendor\ZipStream\Version $version;
     ...
     */
    public function __construct(string $fileName, private readonly Closure $dataCallback, private readonly \MenuManager\Vendor\ZipStream\OperationMode $operationMode, private readonly int $startOffset, private readonly \MenuManager\Vendor\ZipStream\CompressionMethod $compressionMethod, private readonly string $comment, private readonly DateTimeInterface $lastModificationDateTime, private readonly int $deflateLevel, private readonly ?int $maxSize, private readonly ?int $exactSize, private readonly bool $enableZip64, private readonly bool $enableZeroHeader, private readonly Closure $send, private readonly Closure $recordSentBytes)
    {
        $this->fileName = self::filterFilename($fileName);
        $this->checkEncoding();
        if ($this->enableZeroHeader) {
            $this->generalPurposeBitFlag |= \MenuManager\Vendor\ZipStream\GeneralPurposeBitFlag::ZERO_HEADER;
        }
        $this->version = $this->compressionMethod === \MenuManager\Vendor\ZipStream\CompressionMethod::DEFLATE ? \MenuManager\Vendor\ZipStream\Version::DEFLATE : \MenuManager\Vendor\ZipStream\Version::STORE;
    }
    public function cloneSimulationExecution() : self
    {
        return new self($this->fileName, $this->dataCallback, \MenuManager\Vendor\ZipStream\OperationMode::NORMAL, $this->startOffset, $this->compressionMethod, $this->comment, $this->lastModificationDateTime, $this->deflateLevel, $this->maxSize, $this->exactSize, $this->enableZip64, $this->enableZeroHeader, $this->send, $this->recordSentBytes);
    }
    ...
    private function unpackStream()
    {
        ...
        if ($this->operationMode === \MenuManager\Vendor\ZipStream\OperationMode::SIMULATE_STRICT) {
    ...
    {
        if ($this->compressionMethod !== \MenuManager\Vendor\ZipStream\CompressionMethod::STORE) {
            return null;
        }
    ...
    private function addFileHeader() : void
    {
        ...
        $zip64Enabled = $footer !== '';
        if ($zip64Enabled) {
            $this->version = \MenuManager\Vendor\ZipStream\Version::ZIP64;
        }
        if ($this->generalPurposeBitFlag & \MenuManager\Vendor\ZipStream\GeneralPurposeBitFlag::EFS) {
            // Put the tricky entry to
            // force Linux unzip to lookup EFS flag.
            $footer .= \MenuManager\Vendor\ZipStream\Zs\ExtendedInformationExtraField::generate();
        }
        $data = \MenuManager\Vendor\ZipStream\LocalFileHeader::generate(versionNeededToExtract: $this->version->value, generalPurposeBitFlag: $this->generalPurposeBitFlag, compressionMethod: $this->compressionMethod, lastModificationDateTime: $this->lastModificationDateTime, crc32UncompressedData: $this->crc, compressedSize: $zip64Enabled ? 0xffffffff : $this->compressedSize, uncompressedSize: $zip64Enabled ? 0xffffffff : $this->uncompressedSize, fileName: $this->fileName, extraField: $footer);
        ($this->send)($data);
    }
    /**
    ...
    private function checkEncoding() : void
    {
        ...
        if (\mb_check_encoding($this->fileName, 'UTF-8') && \mb_check_encoding($this->comment, 'UTF-8')) {
            $this->generalPurposeBitFlag |= \MenuManager\Vendor\ZipStream\GeneralPurposeBitFlag::EFS;
        }
    }
    private function buildZip64ExtraBlock(bool $force = \false) : string
    {
        ...
        return \MenuManager\Vendor\ZipStream\Zip64\ExtendedInformationExtraField::generate(originalSize: $originalSize, compressedSize: $compressedSize, relativeHeaderOffset: $relativeHeaderOffset, diskStartNumber: null);
    }
    private function addFileFooter() : void
    {
        if (($this->compressedSize > 0xffffffff || $this->uncompressedSize > 0xffffffff) && $this->version !== \MenuManager\Vendor\ZipStream\Version::ZIP64) {
            throw new OverflowException();
        }
        if (!$this->enableZeroHeader) {
            return;
        }
        if ($this->version === \MenuManager\Vendor\ZipStream\Version::ZIP64) {
            $footer = \MenuManager\Vendor\ZipStream\Zip64\DataDescriptor::generate(crc32UncompressedData: $this->crc, compressedSize: $this->compressedSize, uncompressedSize: $this->uncompressedSize);
        } else {
            $footer = \MenuManager\Vendor\ZipStream\DataDescriptor::generate(crc32UncompressedData: $this->crc, compressedSize: $this->compressedSize, uncompressedSize: $this->uncompressedSize);
        }
        ($this->send)($footer);
    }
    ...
    private function compressionInit() : ?DeflateContext
    {
        switch ($this->compressionMethod) {
            case \MenuManager\Vendor\ZipStream\CompressionMethod::STORE:
                // Noting to do
                return null;
            case \MenuManager\Vendor\ZipStream\CompressionMethod::DEFLATE:
    ...
    private function getCdrFile() : string
    {
        ...
        return \MenuManager\Vendor\ZipStream\CentralDirectoryFileHeader::generate(versionMadeBy: \MenuManager\Vendor\ZipStream\ZipStream::ZIP_VERSION_MADE_BY, versionNeededToExtract: $this->version->value, generalPurposeBitFlag: $this->generalPurposeBitFlag, compressionMethod: $this->compressionMethod, lastModificationDateTime: $this->lastModificationDateTime, crc32: $this->crc, compressedSize: $this->compressedSize > 0xffffffff ? 0xffffffff : $this->compressedSize, uncompressedSize: $this->uncompressedSize > 0xffffffff ? 0xffffffff : $this->uncompressedSize, fileName: $this->fileName, extraField: $footer, fileComment: $this->comment, diskNumberStart: 0, internalFileAttributes: 0, externalFileAttributes: 32, relativeOffsetOfLocalHeader: $this->startOffset > 0xffffffff ? 0xffffffff : $this->startOffset);
    }
    private function isSimulation() : bool
    {
        return $this->operationMode === \MenuManager\Vendor\ZipStream\OperationMode::SIMULATE_LAX || $this->operationMode === \MenuManager\Vendor\ZipStream\OperationMode::SIMULATE_STRICT;
    }
}
