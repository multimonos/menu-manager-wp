...
abstract class DefinedName
{
    ...
    public static function createInstance(string $name, ?Worksheet $worksheet = null, ?string $value = null, bool $localOnly = \false, ?Worksheet $scope = null) : self
    {
        ...
        if ($isFormula) {
            return new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\NamedFormula($name, $worksheet, $value, $localOnly, $scope);
        }
        return new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\NamedRange($name, $worksheet, $value, $localOnly, $scope);
    }
    ...
    public function setName(string $name) : self
    {
        if (!empty($name)) {
            ...
            if ($this->worksheet !== null) {
                // New title
                $newTitle = $this->name;
                \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\ReferenceHelper::getInstance()->updateNamedFormulae($this->worksheet->getParentOrThrow(), $oldTitle, $newTitle);
            }
        }
        return $this;
    }
