...
use MenuManager\Vendor\Complex\Complex as ComplexObject;
use MenuManager\Vendor\Complex\Exception as ComplexException;
...
class Complex
{
    ...
    public static function COMPLEX(mixed $realNumber = 0.0, mixed $imaginary = 0.0, mixed $suffix = 'i') : array|string
    {
        ...
        $suffix = $suffix ?? 'i';
        try {
            $realNumber = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engineering\EngineeringValidations::validateFloat($realNumber);
            $imaginary = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engineering\EngineeringValidations::validateFloat($imaginary);
        } catch (Exception $e) {
            return $e->getMessage();
        }
