...
class DateValue
{
    ...
    {
        $PHPDateArray = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::dateParse($dateValue);
        if (!\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::dateParseSucceeded($PHPDateArray)) {
            ...
            \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::adjustYear((string) $testVal1, (string) $testVal2, $testVal3);
            $PHPDateArray = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::dateParse($testVal1 . '-' . $testVal2 . '-' . $testVal3);
            if (!\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::dateParseSucceeded($PHPDateArray)) {
                $PHPDateArray = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::dateParse($testVal2 . '-' . $testVal1 . '-' . $testVal3);
            }
        }
        return $PHPDateArray;
    }
    ...
    private static function finalResults(array $PHPDateArray, DateTimeImmutable $dti, int $baseYear) : string|float|int|DateTime
    {
        ...
        if (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::dateParseSucceeded($PHPDateArray)) {
            // Execute function
            \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::replaceIfEmpty($PHPDateArray['year'], $dti->format('Y'));
            if ($PHPDateArray['year'] < $baseYear) {
                return ExcelError::VALUE();
            }
            \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::replaceIfEmpty($PHPDateArray['month'], $dti->format('m'));
            \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::replaceIfEmpty($PHPDateArray['day'], $dti->format('d'));
            $PHPDateArray['hour'] = 0;
            ...
            if (!\checkdate($month, $day, $year)) {
                return $year === 1900 && $month === 2 && $day === 29 ? \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::returnIn3FormatsFloat(60.0) : ExcelError::VALUE();
            }
            $retValue = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::returnIn3FormatsArray($PHPDateArray, \true);
        }
        return $retValue;
    }
