...
class Days360
{
    ...
    public static function between(mixed $startDate = 0, mixed $endDate = 0, mixed $method = \false) : array|string|int
    {
        ...
        try {
            ...
            $endDate = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::getDateValue($endDate);
        } catch (Exception $e) {
            return $e->getMessage();
        }
        if (!\is_bool($method)) {
    ...
    private static function getStartDay(int $startDay, int $startMonth, int $startYear, bool $methodUS) : int
    {
        if ($startDay == 31) {
            --$startDay;
        } elseif ($methodUS && ($startMonth == 2 && ($startDay == 29 || $startDay == 28 && !\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::isLeapYear($startYear)))) {
            $startDay = 30;
        }
        return $startDay;
    }
