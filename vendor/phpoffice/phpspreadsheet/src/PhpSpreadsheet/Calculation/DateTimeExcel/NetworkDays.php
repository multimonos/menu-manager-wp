...
class NetworkDays
{
    ...
    public static function count(mixed $startDate, mixed $endDate, mixed ...$dateArgs) : array|string|int
    {
        ...
        try {
            ...
            foreach ($dateArgs as $holidayDate) {
                $holidayArray[] = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Helpers::getDateValue($holidayDate);
            }
        } catch (Exception $e) {
            return $e->getMessage();
        }
        ...
        foreach ($holidayArray as $holidayDate) {
            if ($holidayDate >= $startDate && $holidayDate <= $endDate) {
                if (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Week::day($holidayDate, 2) < 6 && !\in_array($holidayDate, $holidayCountedArray)) {
                    --$partWeekDays;
    ...
    {
        $startDow = 6 - (int) \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Week::day($startDate, 2);
        if ($startDow < 0) {
            $startDow = 5;
        }
        return $startDow;
    ...
    {
        $endDow = (int) \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel\Week::day($endDate, 2);
        if ($endDow >= 6) {
            $endDow = 0;
        }
        return $endDow;
