...
class Concatenate
{
    ...
    public static function CONCATENATE(...$args) : string
    {
        ...
        foreach ($aArgs as $arg) {
            $value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractString($arg);
            if (ErrorValue::isError($value, \true)) {
                $returnValue = $value;
                break;
            }
            $returnValue .= \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractString($arg);
    ...
    private static function evaluateTextJoinArray(bool $ignoreEmpty, array &$aArgs) : ?string
    {
        foreach ($aArgs as $key => &$arg) {
            ...
            if ($ignoreEmpty === \true && (\is_string($arg) && \trim($arg) === '' || $arg === null)) {
                ...
                $arg = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::convertBooleanValue($arg);
            }
        }
        return null;
    }
