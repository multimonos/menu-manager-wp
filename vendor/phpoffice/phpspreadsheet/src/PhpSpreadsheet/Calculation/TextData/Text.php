...
class Text
{
    ...
    public static function exact(mixed $value1, mixed $value2) : array|bool|string
    {
        ...
        try {
            $value1 = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractString($value1, \true);
            $value2 = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractString($value2, \true);
        } catch (CalcExp $e) {
            return $e->getMessage();
        }
    ...
    {
        $columnCount = \array_reduce($rows, fn(int $counter, array $row): int => \max($counter, \count($row)), 0);
    ...
    private static function buildDelimiter($delimiter) : string
    {
        ...
        if (\is_array($delimiter) && \count($valueSet) > 1) {
            $quotedDelimiters = \array_map(fn($delimiter): string => \preg_quote($delimiter ?? '', '/'), $valueSet);
