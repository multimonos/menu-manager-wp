...
class Extract
{
    ...
    public static function left(mixed $value, mixed $chars = 1) : array|string
    {
        ...
        try {
            $value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractString($value, \true);
            $chars = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractInt($chars, 0, 1);
        } catch (CalcExp $e) {
            return $e->getMessage();
        }
    ...
    public static function mid(mixed $value, mixed $start, mixed $chars) : array|string
    {
        ...
        try {
            $value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractString($value, \true);
            $start = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractInt($start, 1);
            $chars = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractInt($chars, 0);
        } catch (CalcExp $e) {
            return $e->getMessage();
        }
    ...
    public static function before(mixed $text, $delimiter, mixed $instance = 1, mixed $matchMode = 0, mixed $matchEnd = 0, mixed $ifNotFound = '#N/A') : array|string
    {
        ...
        try {
            ...
            \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractString(Functions::flattenSingleValue($delimiter ?? ''), \true);
        } catch (CalcExp $e) {
            return $e->getMessage();
        }
        ...
        if (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Helpers::extractString(Functions::flattenSingleValue($delimiter ?? '')) === '') {
    ...
    private static function buildDelimiter($delimiter) : string
    {
        if (\is_array($delimiter)) {
            ...
            $quotedDelimiters = \array_map(fn($delimiter): string => \preg_quote($delimiter ?? '', '/'), $delimiter);
