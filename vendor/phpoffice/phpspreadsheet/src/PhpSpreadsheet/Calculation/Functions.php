...
class Functions
{
    ...
    public static function ifCondition(mixed $condition) : string
    {
        ...
        if (!\is_string($condition) || !\in_array($condition[0], ['>', '<', '='], \true)) {
            ...
            if (!\is_numeric($condition)) {
                ...
                $condition = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Calculation::wrapResult(\strtoupper($condition));
            }
        ...
        if (\is_numeric(\trim($operand, '"'))) {
            ...
            $operand = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Calculation::wrapResult(\strtoupper($operand));
    ...
    private static function operandSpecialHandling(mixed $operand) : bool|float|int|string
    {
        ...
        $operand = StringHelper::convertToString($operand);
        if (\strtoupper($operand) === \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Calculation::getTRUE() || \strtoupper($operand) === \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Calculation::getFALSE()) {
