...
class Coupons
{
    ...
    public static function COUPDAYBS(mixed $settlement, mixed $maturity, mixed $frequency, mixed $basis = FinancialConstants::BASIS_DAYS_PER_YEAR_NASD) : string|int|float
    {
        ...
        try {
            ...
            $frequency = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\FinancialValidations::validateFrequency($frequency);
            $basis = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\FinancialValidations::validateBasis($basis);
        } catch (Exception $e) {
            return $e->getMessage();
        }
        $daysPerYear = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\Helpers::daysPerYear(Functions::scalar(DateTimeExcel\DateParts::year($settlement)), $basis);
        ...
        switch ($basis) {
            ...
            case FinancialConstants::BASIS_DAYS_PER_YEAR_ACTUAL:
                ...
                if ($frequency == FinancialConstants::FREQUENCY_ANNUAL) {
                    $daysPerYear = (int) \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\Helpers::daysPerYear(Functions::scalar(DateTimeExcel\DateParts::year($settlement)), $basis);
    ...
    public static function COUPDAYSNC(mixed $settlement, mixed $maturity, mixed $frequency, mixed $basis = FinancialConstants::BASIS_DAYS_PER_YEAR_NASD) : string|float
    {
        ...
        if ($basis === FinancialConstants::BASIS_DAYS_PER_YEAR_NASD) {
            ...
            $settlementEoM = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\Helpers::isLastDayOfMonth($settlementDate);
            if ($settlementEoM) {
                ++$settlement;
            }
        }
    ...
    public static function COUPNCD(mixed $settlement, mixed $maturity, mixed $frequency, mixed $basis = FinancialConstants::BASIS_DAYS_PER_YEAR_NASD) : string|float
    {
        ...
        try {
            ...
            self::validateCouponPeriod($settlement, $maturity);
            $frequency = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\FinancialValidations::validateFrequency($frequency);
            \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\FinancialValidations::validateBasis($basis);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    ...
    private static function couponFirstPeriodDate(float $settlement, float $maturity, int $frequency, bool $next) : float
    {
        ...
        $lastDayFlag = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\Helpers::isLastDayOfMonth($result);
