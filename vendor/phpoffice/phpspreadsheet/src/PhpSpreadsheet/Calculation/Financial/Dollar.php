...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData\Format;
class Dollar
{
    use ArrayEnabled;
    /**
    ...
    public static function decimal(mixed $fractionalDollar = null, mixed $fraction = 0) : array|string|float
    {
        ...
        try {
            $fractionalDollar = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\FinancialValidations::validateFloat(Functions::flattenSingleValue($fractionalDollar) ?? 0.0);
            $fraction = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\FinancialValidations::validateInt(Functions::flattenSingleValue($fraction));
        } catch (Exception $e) {
            return $e->getMessage();
        }
    ...
    public static function fractional(mixed $decimalDollar = null, mixed $fraction = 0) : array|string|float
    {
        ...
        try {
            $decimalDollar = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\FinancialValidations::validateFloat(Functions::flattenSingleValue($decimalDollar) ?? 0.0);
            $fraction = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\FinancialValidations::validateInt(Functions::flattenSingleValue($fraction));
        } catch (Exception $e) {
            return $e->getMessage();
        }
