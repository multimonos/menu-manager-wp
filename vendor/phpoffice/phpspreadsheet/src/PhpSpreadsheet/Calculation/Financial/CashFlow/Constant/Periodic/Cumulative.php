...
namespace MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\Constant\Periodic;

use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\CashFlowValidations;
...
class Cumulative
{
    ...
    public static function interest(mixed $rate, mixed $periods, mixed $presentValue, mixed $start, mixed $end, mixed $type = FinancialConstants::PAYMENT_END_OF_PERIOD) : string|float|int
    {
        ...
        for ($per = $start; $per <= $end; ++$per) {
            $ipmt = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\Constant\Periodic\Interest::payment($rate, $per, $periods, $presentValue, 0, $type);
            if (\is_string($ipmt)) {
                return $ipmt;
            }
            $interest += $ipmt;
    ...
    public static function principal(mixed $rate, mixed $periods, mixed $presentValue, mixed $start, mixed $end, mixed $type = FinancialConstants::PAYMENT_END_OF_PERIOD) : string|float|int
    {
        ...
        $principal = 0;
        for ($per = $start; $per <= $end; ++$per) {
            $ppmt = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\Constant\Periodic\Payments::interestPayment($rate, $per, $periods, $presentValue, 0, $type);
            if (\is_string($ppmt)) {
                return $ppmt;
            }
            $principal += $ppmt;
