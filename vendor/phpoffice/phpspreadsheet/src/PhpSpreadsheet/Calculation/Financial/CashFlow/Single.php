...
class Single
{
    ...
    public static function futureValue(mixed $principal, array $schedule) : string|float
    {
        ...
        try {
            $principal = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\CashFlowValidations::validateFloat($principal);
            foreach ($schedule as $rate) {
                $rate = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\CashFlowValidations::validateFloat($rate);
                $principal *= 1 + $rate;
            }
        } catch (Exception $e) {
            return $e->getMessage();
    ...
    public static function periods(mixed $rate, mixed $presentValue, mixed $futureValue) : string|float
    {
        ...
        try {
            $rate = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\CashFlowValidations::validateRate($rate);
            $presentValue = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\CashFlowValidations::validatePresentValue($presentValue);
            $futureValue = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\CashFlowValidations::validateFutureValue($futureValue);
        } catch (Exception $e) {
            return $e->getMessage();
        }
        // Validate parameters
    ...
    public static function interestRate(mixed $periods = 0.0, mixed $presentValue = 0.0, mixed $futureValue = 0.0) : string|float
    {
        ...
        $futureValue = Functions::flattenSingleValue($futureValue);
        try {
            $periods = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\CashFlowValidations::validateFloat($periods);
            $presentValue = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\CashFlowValidations::validatePresentValue($presentValue);
            $futureValue = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Financial\CashFlow\CashFlowValidations::validateFutureValue($futureValue);
        } catch (Exception $e) {
            return $e->getMessage();
