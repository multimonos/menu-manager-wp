...
class Beta
{
    ...
    public static function distribution(mixed $value, mixed $alpha, mixed $beta, mixed $rMin = 0.0, mixed $rMax = 1.0) : array|string|float
    {
        ...
        try {
            ...
            $alpha = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\DistributionValidations::validateFloat($alpha);
            $beta = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\DistributionValidations::validateFloat($beta);
            $rMax = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\DistributionValidations::validateFloat($rMax);
            $rMin = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\DistributionValidations::validateFloat($rMin);
        } catch (Exception $e) {
            return $e->getMessage();
        }
        if ($rMin > $rMax) {
    ...
    private static function logBeta(float $p, float $q) : float
    {
        if ($p != self::$logBetaCacheP || $q != self::$logBetaCacheQ) {
            ...
            if ($p <= 0.0 || $q <= 0.0 || $p + $q > self::LOG_GAMMA_X_MAX_VALUE) {
                ...
                self::$logBetaCacheResult = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\Gamma::logGamma($p) + \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\Gamma::logGamma($q) - \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\Gamma::logGamma($p + $q);
            }
        }
