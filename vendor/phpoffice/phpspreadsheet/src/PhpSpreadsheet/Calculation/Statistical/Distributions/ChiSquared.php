...
class ChiSquared
{
    ...
    public static function distributionRightTail(mixed $value, mixed $degrees) : array|string|int|float
    {
        ...
        try {
            ...
            $degrees = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\DistributionValidations::validateInt($degrees);
        } catch (Exception $e) {
            return $e->getMessage();
        }
        if ($degrees < 1) {
        ...
        return 1 - \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\Gamma::incompleteGamma($degrees / 2, $value / 2) / \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\Gamma::gammaValue($degrees / 2);
    }
    /**
     * CHIDIST.
     *
    ...
    public static function distributionLeftTail(mixed $value, mixed $degrees, mixed $cumulative) : array|string|int|float
    {
        ...
        return $value ** ($degrees / 2 - 1) * \exp(-$value / 2) / (2 ** ($degrees / 2) * \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\Gamma::gammaValue($degrees / 2));
    }
    /**
     * CHIINV.
     *
    ...
    public static function inverseRightTail(mixed $probability, mixed $degrees)
    {
        ...
        $callback = fn($value): float => 1 - \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\Gamma::incompleteGamma($degrees / 2, $value / 2) / \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\Gamma::gammaValue($degrees / 2);
        $newtonRaphson = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\NewtonRaphson($callback);
    ...
    {
        /** @var float $gln */
        $gln = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Distributions\Gamma::ln($n / 2);
        $a = 0.5 * $n;
        $ap = $a;
        $sum = 1.0 / $a;
        $del = $sum;
