...
class Percentiles
{
    ...
    public static function PERCENTILE(mixed ...$args)
    {
        ...
        try {
            $entry = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\StatisticalValidations::validateFloat($entry);
        } catch (Exception $e) {
            return $e->getMessage();
        }
        if ($entry < 0 || $entry > 1) {
        ...
        if ($mValueCount > 0) {
            \sort($mArgs);
            $count = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\Counts::COUNT($mArgs);
            $index = $entry * ($count - 1);
    ...
    public static function PERCENTRANK(mixed $valueSet, mixed $value, mixed $significance = 3) : string|float
    {
        ...
        try {
            $value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\StatisticalValidations::validateFloat($value);
            $significance = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\StatisticalValidations::validateInt($significance);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    ...
    public static function RANK(mixed $value, mixed $valueSet, mixed $order = self::RANK_SORT_DESCENDING)
    {
        ...
        try {
            $value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\StatisticalValidations::validateFloat($value);
            $order = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\StatisticalValidations::validateInt($order);
        } catch (Exception $e) {
            return $e->getMessage();
        }
        $valueSet = self::rankFilterValues($valueSet);
