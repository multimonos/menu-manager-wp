...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\Trend\Trend;
class Trends
{
    use ArrayEnabled;
    ...
    public static function FORECAST(mixed $xValue, array $yValues, array $xValues)
    {
        ...
        try {
            $xValue = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical\StatisticalValidations::validateFloat($xValue);
            self::checkTrendArrays($yValues, $xValues);
            self::validateTrendArrays($yValues, $xValues);
        } catch (Exception $e) {
            return $e->getMessage();
