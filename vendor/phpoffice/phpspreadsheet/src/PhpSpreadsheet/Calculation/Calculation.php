...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\BranchPruner;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\CyclicReferenceStack;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\Logger;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\Operands;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Information\ExcelError;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Token\Stack;
...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\NamedRange;
...
use Throwable;
class Calculation extends \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\CalculationLocale
{
    /** Constants                */
    /** Regular Expressions        */
    //    Numeric operand
     ...
     */
    private static ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Calculation $instance = null;
    /**
     ...
     */
    private static array $controlFunctions = ['MKMATRIX' => ['argumentCount' => '*', 'functionCall' => [\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Internal\MakeMatrix::class, 'make']], 'NAME.ERROR' => ['argumentCount' => '*', 'functionCall' => [ExcelError::class, 'NAME']], 'WILDCARDMATCH' => ['argumentCount' => '2', 'functionCall' => [\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Internal\WildcardMatch::class, 'compare']]];
    ...
    {
        throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception('Cloning the calculation engine is not allowed!');
    }
    /**
    ...
    public function calculate(?Cell $cell = null) : mixed
    {
        try {
            ...
            throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception($e->getMessage());
        }
    }
    /**
    ...
    public function calculateCellValue(?Cell $cell = null, bool $resetLog = \true) : mixed
    {
        ...
        try {
            ...
            if ($this->spreadsheet === null) {
                throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception('null spreadsheet in calculateCellValue');
            }
            $cellAddressAttempted = \true;
            $cellAddress = \array_pop($this->cellStack);
            if ($cellAddress === null) {
                throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception('null cellAddress in calculateCellValue');
            }
            $testSheet = $this->spreadsheet->getSheetByName($cellAddress['sheet']);
            if ($testSheet === null) {
                throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception('worksheet not found in calculateCellValue');
            }
            ...
            throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception($e->getMessage(), $e->getCode(), $e);
        }
        if (\is_array($result) && $this->getInstanceArrayReturnType() !== self::RETURN_ARRAY_AS_ARRAY) {
            $testResult = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::flattenArray($result);
    ...
    private function showValue(mixed $value) : mixed
    {
        if ($this->debugLog->getWriteDebugLog()) {
            $testArray = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::flattenArray($value);
        ...
        return \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::flattenSingleValue($value);
    }
    /**
        ...
        while (\true) {
            ...
            if ($opCharacter === '-' && !$expectingOperator) {
                ...
                if (\is_array($d) && \preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/miu', $d['value'], $matches)) {
                    ...
                    } catch (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception $e) {
    ...
    private function processTokenStack(false|array $tokens, ?string $cellID = null, ?Cell $cell = null)
    {
        ...
        foreach ($tokens as $tokenIdx => $tokenData) {
            ...
            if ($this->branchPruningEnabled && isset($tokenData['onlyIf'])) {
                ...
                $storeValueAsBool = $storeValue === null ? \true : (bool) \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::flattenSingleValue($storeValue);
                if (\is_array($storeValue)) {
                    $wrappedItem = \end($storeValue);
                    $storeValue = \is_array($wrappedItem) ? \end($wrappedItem) : $wrappedItem;
                }
                if ((isset($storeValue) || $tokenData['reference'] === 'NULL') && (!$storeValueAsBool || \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Information\ErrorValue::isError($storeValue) || $storeValue === 'Pruned branch')) {
            ...
            if ($token instanceof Operands\StructuredReference) {
                ...
                try {
                    ...
                    if ($e->getCode() === \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception::CALCULATION_ENGINE_PUSH_TO_STACK) {
                ...
                switch ($token) {
                    ...
                    case ':':
                        ...
                        $sheet1 ??= '';
                        [$sheet2, $operand2Data['reference']] = Worksheet::extractSheetTitle($operand2Data['reference'], \true, \true);
                        if (empty($sheet2)) {
                            $sheet2 = $sheet1;
                        }
                        if ($sheet1 === $sheet2) {
                        ...
                        if (\is_array($operand1) || \is_array($operand2)) {
                            ...
                            for ($row = 0; $row < $rows; ++$row) {
                                for ($column = 0; $column < $columns; ++$column) {
                                    ...
                                    if (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Information\ErrorValue::isError($op1x)) {
                                        // no need to do anything
                                    } elseif (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Information\ErrorValue::isError($op2x)) {
                            ...
                            if (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Information\ErrorValue::isError($operand1)) {
                                $result = $operand1;
                            } elseif (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Information\ErrorValue::isError($operand2)) {
                                $result = $operand2;
                            } else {
                    ...
                    case '∩':
                        ...
                        if (\count(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::flattenArray($cellIntersect)) === 0) {
                ...
                if (isset($phpSpreadsheetFunctions[$functionName]) || isset(self::$controlFunctions[$functionName])) {
                    ...
                    if ($functionName !== 'MKMATRIX') {
                        if ($this->debugLog->getWriteDebugLog()) {
                            \krsort($argArrayVals);
                            $this->debugLog->writeDebugLog('Evaluating %s ( %s )', self::localeFunc($functionName), \implode(self::$localeArgumentSeparator . ' ', \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::flattenArray($argArrayVals)));
                        }
                    }
                    ...
                    if (!\is_array($functionCall)) {
                        foreach ($args as &$arg) {
                            $arg = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::flattenSingleValue($arg);
                        }
                        unset($arg);
                    }
                ...
                if (isset(self::EXCEL_CONSTANTS[\strtoupper($token ?? '')])) {
                    ...
                    if (\str_starts_with($definedName, '_xleta')) {
                        return \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::NOT_YET_IMPLEMENTED;
                    }
        ...
        if (\is_string($operand)) {
            ...
            if (!\is_numeric($operand)) {
                ...
                if ($operand > '' && $operand[0] == '#') {
                    ...
                    return \false;
                } elseif (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\FormattedNumber::convertToNumberIfFormatted($operand) === \false) {
    ...
    private function executeArrayComparison(mixed $operand1, mixed $operand2, string $operation, Stack &$stack, bool $recursingArrays) : array
    {
        ...
        if (!\is_array($operand2) && \is_array($operand1)) {
            ...
            throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception('Neither operand is an arra');
        }
    ...
    private function executeBinaryComparisonOperation(mixed $operand1, mixed $operand2, string $operation, Stack &$stack, bool $recursingArrays = \false) : array|bool
    {
        ...
        $result = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\BinaryComparison::compare($operand1, $operand2, $operation);
        //    Log the result details
        $this->debugLog->writeDebugLog('Evaluation Result is %s', $this->showTypeDetails($result));
        //    And push the result onto the stack
        $stack->push('Value', $result);
    ...
    private function executeNumericBinaryOperation(mixed $operand1, mixed $operand2, string $operation, Stack &$stack) : mixed
    {
        ...
        if (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::getCompatibilityMode() != \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::COMPATIBILITY_OPENOFFICE && (\is_string($operand1) && !\is_numeric($operand1) && $operand1 !== '' || \is_string($operand2) && !\is_numeric($operand2) && $operand2 !== '')) {
            $result = ExcelError::VALUE();
        } elseif (\is_array($operand1) || \is_array($operand2)) {
            //    Ensure that both operands are arrays/matrices
            if (\is_array($operand1)) {
                foreach ($operand1 as $key => $value) {
                    $operand1[$key] = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::flattenArray($value);
                }
            }
            if (\is_array($operand2)) {
                foreach ($operand2 as $key => $value) {
                    $operand2[$key] = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Functions::flattenArray($value);
                }
            }
                ...
                for ($column = 0; $column < $columns; ++$column) {
                    ...
                    switch ($operation) {
                        ...
                        case '^':
                            ...
                            break;
                        default:
                            throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception('Unsupported numeric binary operation');
                    }
                }
            }
            $result = $operand1;
    ...
    protected function raiseFormulaError(string $errorMessage, int $code = 0, ?Throwable $exception = null) : bool
    {
        ...
        if (!$suppress) {
            throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception($errorMessage, $code, $exception);
        }
        return \false;
    }
    /**
     ...
     */
    public function extractCellRange(string &$range = 'A1', ?Worksheet $worksheet = null, bool $resetLog = \true) : array
    {
        // Return value
        $returnValue = [];
        if ($worksheet !== null) {
    ...
    {
        return \is_string($operand) && \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Information\ErrorValue::isError($operand) ? $operand : ExcelError::VALUE();
    }
