...
class Lookup
{
    ...
    public static function lookup(mixed $lookupValue, mixed $lookupVector, $resultVector = null) : mixed
    {
        ...
        if ($lookupRows === 1 && $lookupColumns > 1 || !$hasResultVector && $lookupRows === 2 && $lookupColumns !== 2) {
            $lookupVector = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\LookupRef\Matrix::transpose($lookupVector);
            $lookupRows = self::rowCount($lookupVector);
            $lookupColumns = self::columnCount($lookupVector);
        }
        ...
        return \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\LookupRef\VLookup::lookup($lookupValue, $lookupVector, 2);
    }
    ...
    private static function verifyResultVector(array $resultVector) : array
    {
        ...
        if ($resultRows === 1 && $resultColumns > 1) {
            $resultVector = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\LookupRef\Matrix::transpose($resultVector);
        }
        return $resultVector;
    }
