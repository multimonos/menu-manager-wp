...
class Unique
{
    ...
    private static function uniqueByColumn(array $lookupVector, bool $exactlyOnce) : mixed
    {
        ...
        if (\count($lookupVector, \COUNT_RECURSIVE) > \count($flattenedLookupVector, \COUNT_RECURSIVE) + 1) {
            ...
            $transpose = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\LookupRef\Matrix::transpose($lookupVector);
            $result = self::uniqueByRow($transpose, $exactlyOnce);
            return \is_array($result) ? \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\LookupRef\Matrix::transpose($result) : $result;
        }
