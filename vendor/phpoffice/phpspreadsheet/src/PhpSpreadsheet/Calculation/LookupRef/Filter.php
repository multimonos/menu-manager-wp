...
class Filter
{
    public static function filter(array $lookupArray, mixed $matchArray, mixed $ifEmpty = null) : mixed
    {
        ...
        $result = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\LookupRef\Matrix::isColumnVector($matchArray) ? self::filterByRow($lookupArray, $matchArray) : self::filterByColumn($lookupArray, $matchArray);
        if (empty($result)) {
    ...
    private static function filterByRow(array $lookupArray, array $matchArray) : array
    {
        ...
        // @phpstan-ignore-line
        return \array_filter(\array_values($lookupArray), fn($index): bool => (bool) $matchArray[$index], \ARRAY_FILTER_USE_KEY);
    }
    private static function filterByColumn(array $lookupArray, array $matchArray) : array
    {
        $lookupArray = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\LookupRef\Matrix::transpose($lookupArray);
        ...
        return \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\LookupRef\Matrix::transpose($result);
    }
}
