...
class Base
{
    ...
    public static function evaluate(mixed $number, mixed $radix, mixed $minLength = null) : array|string
    {
        ...
        try {
            $number = (float) \floor(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullBool($number));
            $radix = (int) \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullBool($radix);
        } catch (Exception $e) {
            return $e->getMessage();
        }
