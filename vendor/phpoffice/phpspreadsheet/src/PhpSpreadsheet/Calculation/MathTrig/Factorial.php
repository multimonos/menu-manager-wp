...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical;
class Factorial
{
    use ArrayEnabled;
    /**
    ...
    public static function fact($factVal) : array|string|float|int
    {
        ...
        try {
            $factVal = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullBool($factVal);
            \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNotNegative($factVal);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    ...
    public static function factDouble($factVal) : array|string|float|int
    {
        ...
        try {
            $factVal = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullSubstitution($factVal, 0);
            \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNotNegative($factVal);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    ...
    public static function multinomial(...$args) : string|int|float
    {
        ...
        try {
            ...
            foreach (Functions::flattenArray($args) as $argx) {
                $arg = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullSubstitution($argx, null);
                \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNotNegative($arg);
                $arg = (int) $arg;
                $summer += $arg;
