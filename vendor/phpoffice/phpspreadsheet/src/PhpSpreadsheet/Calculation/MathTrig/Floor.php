...
class Floor
{
    ...
    public static function floor(mixed $number, mixed $significance = null)
    {
        ...
        try {
            ...
            $significance = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullSubstitution($significance, $number < 0 ? -1 : 1);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    ...
    public static function math(mixed $number, mixed $significance = null, mixed $mode = 0)
    {
        ...
        try {
            $number = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullBool($number);
            $significance = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullSubstitution($significance, $number < 0 ? -1 : 1);
            $mode = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullSubstitution($mode, null);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    ...
    public static function precise($number, $significance = 1)
    {
        ...
        try {
            $number = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullBool($number);
            $significance = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::validateNumericNullSubstitution($significance, null);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    ...
    {
        return \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::returnSign($significance) == -1 || \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::returnSign($number) == -1 && !empty($mode);
    }
    /**
     * Avoid Scrutinizer problems concerning complexity.
     */
    private static function argumentsOk(float $number, float $significance) : string|float
    {
        ...
        if (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::returnSign($significance) == 1) {
            return \floor($number / $significance) * $significance;
        }
        if (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::returnSign($number) == -1 && \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\MathTrig\Helpers::returnSign($significance) == -1) {
            return \floor($number / $significance) * $significance;
        }
