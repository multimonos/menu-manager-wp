...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Chart\Renderer\IRenderer;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Collection\Memory;
use MenuManager\Vendor\Psr\Http\Client\ClientInterface;
use MenuManager\Vendor\Psr\Http\Message\RequestFactoryInterface;
use MenuManager\Vendor\Psr\SimpleCache\CacheInterface;
use ReflectionClass;
class Settings
{
    /**
    ...
    public static function setChartRenderer(string $rendererClassName) : void
    {
        if (!\is_a($rendererClassName, IRenderer::class, \true)) {
            throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Exception('Chart renderer must implement ' . IRenderer::class);
        }
    ...
    public static function getHttpClient() : ClientInterface
    {
        if (!self::$httpClient || !self::$requestFactory) {
            throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Exception('HTTP client must be configured via Settings::setHttpClient() to be able to use WEBSERVICE function.');
        }
