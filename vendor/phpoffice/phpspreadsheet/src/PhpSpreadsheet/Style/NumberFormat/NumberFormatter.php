...
class NumberFormatter extends \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\NumberFormat\BaseFormatter
{
    ...
    public static function format(mixed $value, string $format) : string
    {
        ...
        if (\preg_match('/[#\\?0]?.*[#\\?0]\\/(\\?+|\\d+|#)/', $format)) {
            ...
            $format = (string) \preg_replace('/\\/[#0]+/', '/?', $format);
            $value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\NumberFormat\FractionFormatter::format($value, $format);
        } else {
