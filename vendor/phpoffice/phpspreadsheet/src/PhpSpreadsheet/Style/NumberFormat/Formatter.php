...
namespace MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\NumberFormat;

use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Calculation;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color\BIFF8;
...
class Formatter extends \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\NumberFormat\BaseFormatter
{
    /**
    ...
    public static function toFormattedString($value, string $format, null|array|callable $callBack = null) : string
    {
        ...
        if (\preg_match(self::SECTION_SPLIT, $format) === 0 && \preg_match(self::SYMBOL_AT, $formatx) === 1) {
            ...
            $value = \str_replace(['$', '"'], ['\\$', self::QUOTE_REPLACEMENT], StringHelper::convertToString($value));
        ...
        if (\preg_match('/(\\[\\$[A-Z]*-[0-9A-F]*\\])*[hmsdy](?=(?:[^"]|"[^"]*")*$)/miu', $format) && !\preg_match('/\\[\\$[A-Z]{3}\\]/miu', $format) && \preg_match('/[0\\?#]\\.(?![^\\[]*\\])/miu', $format) === 0) {
            // datetime format
            $value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\NumberFormat\DateFormatter::format($value, $format);
        } else {
            if (\str_starts_with($format, '"') && \str_ends_with($format, '"') && \substr_count($format, '"') === 2) {
                ...
                $value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\NumberFormat\PercentageFormatter::format(0 + (float) $value, $format);
            } else {
                $value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\NumberFormat\NumberFormatter::format($value, $format);
            }
        }
