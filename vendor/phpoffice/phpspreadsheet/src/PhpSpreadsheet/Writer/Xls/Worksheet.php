...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\RichText\Run;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\StringHelper;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\Xls;
...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Exception as WriterException;
...
// */
class Worksheet extends \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\BIFFwriter
{
     ...
     * Formula parser.
     */
    private \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Parser $parser;
    /**
     ...
     * Sheet object.
     */
    public \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpSheet;
    /**
     * Escher object corresponding to MSODRAWING.
     */
    private ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\Escher $escher = null;
    /**
    ...
    private ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Workbook $writerWorkbook;
    /**
     * Constructor.
     *
     ...
     */
    public function __construct(int &$str_total, int &$str_unique, array &$str_table, array &$colors, \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Parser $parser, bool $preCalculateFormulas, \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpSheet, ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Workbook $writerWorkbook = null)
    {
    ...
    public function close() : void
    {
        ...
        if (($defaultWidth = $phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
            $defaultWidth = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\Font::getDefaultColumnWidthByFont($phpSheet->getParentOrThrow()->getDefaultStyle()->getFont());
        }
            ...
            if ($cVal instanceof RichText) {
                ...
                switch ($cell->getDatatype()) {
                    ...
                    case DataType::TYPE_ERROR:
                        $this->writeBoolErr($row, $column, \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\ErrorCode::error($cell->getValueString()), 1, $xfIndex);
                        break;
                }
            }
        }
    ...
    {
        $conditionalFormulaHelper = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\ConditionalHelper($this->parser);
        ...
        if (isset($calculatedValue)) {
            ...
            if (\is_bool($calculatedValue)) {
                ...
                if (isset($errorCodes[$calculatedValue])) {
                    // Error value
                    $num = \pack('CCCvCv', 0x2, 0x0, \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\ErrorCode::error($calculatedValue), 0x0, 0x0, 0xffff);
     ...
     * Get Escher object.
     */
    public function getEscher() : ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\Escher
    {
        return $this->escher;
    }
    /**
     * Set Escher object.
     */
    public function setEscher(?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\Escher $escher) : void
    {
        $this->escher = $escher;
    }
    /**
    ...
    private function writeMsoDrawing() : void
    {
        ...
        if (isset($this->escher)) {
            $writer = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Escher($this->escher);
            $data = $writer->close();
    ...
    private function writeDataValidity() : void
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
                ...
                $options = 0x0;
                // data type
                $type = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\CellDataValidation::type($dataValidation);
                $options |= $type << 0;
                // error style
                $errorStyle = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\CellDataValidation::errorStyle($dataValidation);
                ...
                // condition operator
                $operator = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\CellDataValidation::operator($dataValidation);
                $options |= $operator << 20;
                $data = \pack('V', $options);
                // prompt title
     ...
     */
    private function writeCFRule(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\ConditionalHelper $conditionalFormulaHelper, Conditional $conditional, string $cellRange) : void
    {
        $record = 0x1b1;
        // Record identifier
        $type = null;
        ...
        if ($bFormatFont == 1) {
            ...
            switch ($conditional->getStyle()->getFont()->getUnderline()) {
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_NONE:
                    $dataBlockFont .= \pack('C', 0x0);
                    $fontUnderline = 0;
                    break;
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_DOUBLE:
                    $dataBlockFont .= \pack('C', 0x2);
                    $fontUnderline = 0;
                    break;
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_DOUBLEACCOUNTING:
                    $dataBlockFont .= \pack('C', 0x22);
                    $fontUnderline = 0;
                    break;
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE:
                    $dataBlockFont .= \pack('C', 0x1);
                    $fontUnderline = 0;
                    break;
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLEACCOUNTING:
        ...
        if ($bFormatBorder === 1) {
            $blockLineStyle = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Style\CellBorder::style($conditional->getStyle()->getBorders()->getLeft());
            $blockLineStyle |= \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Style\CellBorder::style($conditional->getStyle()->getBorders()->getRight()) << 4;
            $blockLineStyle |= \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Style\CellBorder::style($conditional->getStyle()->getBorders()->getTop()) << 8;
            $blockLineStyle |= \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Style\CellBorder::style($conditional->getStyle()->getBorders()->getBottom()) << 12;
            if ($bBorderLeft !== 0) {
        ...
        if ($bFormatFill === 1) {
            // Fill Pattern Style
            $blockFillPatternStyle = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Style\CellFill::style($conditional->getStyle()->getFill());
            // Background Color
