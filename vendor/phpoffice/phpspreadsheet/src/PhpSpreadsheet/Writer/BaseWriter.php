...
abstract class BaseWriter implements \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\IWriter
{
    /**
    ...
    public function setUseDiskCaching(bool $useDiskCache, ?string $cacheDirectory = null) : self
    {
        ...
        if ($cacheDirectory !== null) {
            if (\is_dir($cacheDirectory)) {
                ...
                throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Exception("Directory does not exist: {$cacheDirectory}");
            }
        }
        return $this;
    }
    ...
    public function openFileHandle($filename) : void
    {
        ...
        if ($fileHandle === \false) {
            throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Exception('Could not open file "' . $filename . '" for writing.');
        }
    ...
    protected function maybeCloseFileHandle() : void
    {
        if ($this->shouldCloseFile) {
            if (!\fclose($this->fileHandle)) {
                throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Exception('Could not close file after writing.');
            }
        }
    }
}
