...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Table as WorksheetTable;
class Table extends \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xlsx\WriterPart
{
    /**
        ...
        if ($table->getShowHeaderRow() && $table->getAllowFilter() === \true) {
            ...
            foreach (\range($rangeStart[0], $rangeEnd[0]) as $offset => $columnIndex) {
                ...
                if (!$column->getShowFilterButton()) {
                    ...
                    \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xlsx\AutoFilter::writeAutoFilterColumn($objWriter, $column, $offset);
                }
            }
            $objWriter->endElement();
            // autoFilter
