...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\XMLWriter;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\Conditional;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\ConditionalColorScale;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\ConditionalDataBar;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\ConditionalFormattingRuleExtension;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\RowDimension;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\SheetView;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet as PhpspreadsheetWorksheet;
class Worksheet extends \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xlsx\WriterPart
{
    ...
    private static function writeOtherCondElements(XMLWriter $objWriter, Conditional $conditional, string $cellCoordinate) : void
    {
        ...
        if ($conditional->getConditionType() == Conditional::CONDITION_CELLIS || $conditional->getConditionType() == Conditional::CONDITION_EXPRESSION || !empty($conditions)) {
            foreach ($conditions as $formula) {
                ...
                $objWriter->writeElement('formula', \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xlsx\FunctionPrefix::addFunctionPrefix("{$formula}"));
            }
        } else {
    ...
    private function writeConditionalFormatting(XMLWriter $objWriter, PhpspreadsheetWorksheet $worksheet) : void
    {
        ...
        foreach ($worksheet->getConditionalStylesCollection() as $cellCoordinate => $conditionalStyles) {
            ...
            foreach ($conditionalStyles as $conditional) {
                ...
                self::writeAttributeIf($objWriter, $conditional->getConditionType() !== Conditional::CONDITION_COLORSCALE && $conditional->getConditionType() !== Conditional::CONDITION_DATABAR && $conditional->getNoFormatSet() === \false, 'dxfId', (string) $this->getParentWriter()->getStylesConditionalHashTable()->getIndexForHashCode($conditional->getHashCode()));
    ...
    private function writeDataValidations(XMLWriter $objWriter, PhpspreadsheetWorksheet $worksheet) : void
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            foreach ($dataValidationCollection as $coordinate => $dv) {
                ...
                if ($dv->getFormula1() !== '') {
                    $objWriter->writeElement('formula1', \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xlsx\FunctionPrefix::addFunctionPrefix($dv->getFormula1()));
                }
                if ($dv->getFormula2() !== '') {
                    $objWriter->writeElement('formula2', \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xlsx\FunctionPrefix::addFunctionPrefix($dv->getFormula2()));
                }
                $objWriter->endElement();
            }
            $objWriter->endElement();
    ...
    {
        \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xlsx\AutoFilter::writeAutoFilter($objWriter, $worksheet);
    }
    /**
     * Write Table.
     */
    ...
    private function writeCellError(XMLWriter $objWriter, string $mappedType, string $cellValue, string $formulaerr = '#NULL!') : void
    {
        ...
        self::writeElementIf($objWriter, $cellIsFormula, 'f', \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xlsx\FunctionPrefix::addFunctionPrefixStripEquals($cellValue));
    ...
    private function writeCellFormula(XMLWriter $objWriter, string $cellValue, Cell $cell) : void
    {
        ...
        if ($calculatedValue === ExcelError::SPILL()) {
            ...
            $objWriter->text(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xlsx\FunctionPrefix::addFunctionPrefixStripEquals($cellValue));
            $objWriter->endElement();
            // f
        ...
        if (($attributes['t'] ?? null) === 'array') {
            ...
            $objWriter->writeElement('f', \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xlsx\FunctionPrefix::addFunctionPrefixStripEquals($cellValue));
