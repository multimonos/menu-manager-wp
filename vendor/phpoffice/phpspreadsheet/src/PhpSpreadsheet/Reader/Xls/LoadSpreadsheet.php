...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\Escher as SharedEscher;
...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\Xls as SharedXls;
...
class LoadSpreadsheet extends Xls
{
    ...
    protected function loadSpreadsheetFromFile2(string $filename, Xls $xls) : Spreadsheet
    {
        ...
        if (!$xls->readDataOnly) {
            foreach ($xls->objFonts as $objFont) {
                if (isset($objFont->colorIndex)) {
                    $color = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($objFont->colorIndex, $xls->palette, $xls->version);
            ...
            foreach ($xls->spreadsheet->getCellXfCollection() as $objStyle) {
                ...
                if (isset($fill->startcolorIndex)) {
                    $startColor = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($fill->startcolorIndex, $xls->palette, $xls->version);
                    $fill->getStartColor()->setRGB($startColor['rgb']);
                }
                if (isset($fill->endcolorIndex)) {
                    $endColor = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($fill->endcolorIndex, $xls->palette, $xls->version);
                ...
                if (isset($top->colorIndex)) {
                    $borderTopColor = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($top->colorIndex, $xls->palette, $xls->version);
                    $top->getColor()->setRGB($borderTopColor['rgb']);
                }
                if (isset($right->colorIndex)) {
                    $borderRightColor = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($right->colorIndex, $xls->palette, $xls->version);
                    $right->getColor()->setRGB($borderRightColor['rgb']);
                }
                if (isset($bottom->colorIndex)) {
                    $borderBottomColor = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($bottom->colorIndex, $xls->palette, $xls->version);
                    $bottom->getColor()->setRGB($borderBottomColor['rgb']);
                }
                if (isset($left->colorIndex)) {
                    $borderLeftColor = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($left->colorIndex, $xls->palette, $xls->version);
                    $left->getColor()->setRGB($borderLeftColor['rgb']);
                }
                if (isset($diagonal->colorIndex)) {
                    $borderDiagonalColor = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($diagonal->colorIndex, $xls->palette, $xls->version);
        ...
        if (!$xls->readDataOnly && $xls->drawingGroupData) {
            ...
            $reader = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Escher($escher);
        ...
        foreach ($xls->sheets as $sheet) {
            ...
            if (!$xls->readDataOnly && $xls->drawingData) {
                ...
                $reader = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Escher($escherWorksheet);
