...
class ConditionalFormatting extends Xls
{
    ...
    protected function readCFHeader2(Xls $xls) : array
    {
        ...
        $cellRangeAddressList = $xls->version == self::XLS_BIFF8 ? \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Biff8::readBIFF8CellRangeAddressList(\substr($recordData, 12)) : \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Biff5::readBIFF5CellRangeAddressList(\substr($recordData, 12));
    ...
    private function getCFFontStyle(string $options, Style $style, Xls $xls) : void
    {
        ...
        if ($color !== -1) {
            $style->getFont()->getColor()->setRGB(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($color, $xls->palette, $xls->version)['rgb']);
        }
    }
    ...
    private function getCFBorderStyle(string $options, Style $style, bool $hasBorderLeft, bool $hasBorderRight, bool $hasBorderTop, bool $hasBorderBottom, Xls $xls) : void
    {
        ...
        if ($hasBorderLeft) {
            ...
            $style->getBorders()->getLeft()->getColor()->setRGB(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($leftc, $xls->palette, $xls->version)['rgb']);
        }
        if ($hasBorderRight) {
            $style->getBorders()->getRight()->setBorderStyle(self::BORDER_STYLE_MAP[$right]);
            $style->getBorders()->getRight()->getColor()->setRGB(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($rightc, $xls->palette, $xls->version)['rgb']);
        }
        if ($hasBorderTop) {
            $style->getBorders()->getTop()->setBorderStyle(self::BORDER_STYLE_MAP[$top]);
            $style->getBorders()->getTop()->getColor()->setRGB(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($topc, $xls->palette, $xls->version)['rgb']);
        }
        if ($hasBorderBottom) {
            $style->getBorders()->getBottom()->setBorderStyle(self::BORDER_STYLE_MAP[$bottom]);
            $style->getBorders()->getBottom()->getColor()->setRGB(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($bottomc, $xls->palette, $xls->version)['rgb']);
        }
    }
    private function getCFFillStyle(string $options, Style $style, Xls $xls) : void
    {
        ...
        if ($fillPattern !== Fill::FILL_NONE) {
            ...
            if ($fillPattern === Fill::FILL_SOLID) {
                $style->getFill()->getStartColor()->setRGB(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($color2, $xls->palette, $xls->version)['rgb']);
            } else {
                $style->getFill()->getStartColor()->setRGB(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($color1, $xls->palette, $xls->version)['rgb']);
                $style->getFill()->getEndColor()->setRGB(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color::map($color2, $xls->palette, $xls->version)['rgb']);
            }
        }
    }
