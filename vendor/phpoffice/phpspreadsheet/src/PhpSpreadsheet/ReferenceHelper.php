...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter;
...
class ReferenceHelper
{
     ...
     */
    private static ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\ReferenceHelper $instance = null;
    private ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\CellReferenceHelper $cellReferenceHelper = null;
    /**
    ...
    protected function adjustProtectedCells(Worksheet $worksheet, int $numberOfColumns, int $numberOfRows) : void
    {
        ...
        foreach ($aProtectedCells as $cellAddress => $protectedRange) {
            ...
            if ($cellAddress !== $newReference) {
                ...
                if ($newReference) {
                    $worksheet->protectCells($newReference, $protectedRange->getPassword(), \true);
                }
            }
        }
    }
    ...
    public function insertNewBefore(string $beforeCellAddress, int $numberOfColumns, int $numberOfRows, Worksheet $worksheet) : void
    {
        ...
        if ($this->cellReferenceHelper === null || $this->cellReferenceHelper->refreshRequired($beforeCellAddress, $numberOfColumns, $numberOfRows)) {
            $this->cellReferenceHelper = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\CellReferenceHelper($beforeCellAddress, $numberOfColumns, $numberOfRows);
        }
     ...
     */
    public function updateNamedFormulae(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Spreadsheet $spreadsheet, string $oldName = '', string $newName = '') : void
    {
        if ($oldName == '') {
            return;
        }
    ...
    private function updateNamedRange(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\DefinedName $definedName, Worksheet $worksheet, string $beforeCellAddress, int $numberOfColumns, int $numberOfRows) : void
    {
    ...
    private function updateNamedFormula(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\DefinedName $definedName, Worksheet $worksheet, string $beforeCellAddress, int $numberOfColumns, int $numberOfRows) : void
    {
        if ($definedName->getWorksheet() !== null && $definedName->getWorksheet()->getHashInt() === $worksheet->getHashInt()) {
            /**
    ...
    private function updateCellRange(string $cellRange = 'A1:A1', bool $includeAbsoluteReferences = \false, bool $onlyAbsoluteReferences = \false) : string
    {
        if (!Coordinate::coordinateIsRange($cellRange)) {
            throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Exception('Only cell ranges may be passed to this method.');
        }
        // Update range
    ...
    private function adjustAutoFilterDelete(int $startCol, int $numberOfColumns, int $rangeEnd, AutoFilter $autoFilter) : void
    {
        ...
        do {
            ...
            ++$toColID;
            ++$startColID;
            // this confuses phpstan into thinking startColID is int/float
    ...
    {
        throw new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Exception('Cloning a Singleton is not allowed!');
    }
}
