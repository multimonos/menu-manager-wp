...
use Stringable;
class StringValueBinder extends \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DefaultValueBinder implements \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\IValueBinder
{
    ...
    public function bindValue(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Cell $cell, mixed $value) : bool
    {
        ...
        if ($value === null && $this->convertNull === \false) {
            $cell->setValueExplicit($value, \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NULL);
        } elseif (\is_bool($value) && $this->convertBoolean === \false) {
            $cell->setValueExplicit($value, \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_BOOL);
        ...
        } elseif (\is_string($value) && \strlen($value) > 1 && $value[0] === '=' && $this->convertFormula === \false && parent::dataTypeForValue($value) === \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_FORMULA) {
            $cell->setValueExplicit($value, \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_FORMULA);
        } else {
            $ignoredErrors = \is_numeric($value);
            $cell->setValueExplicit((string) $value, \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
        }
    ...
    protected function bindObjectValue(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Cell $cell, object $value) : bool
    {
        ...
        if ($value instanceof DateTimeInterface) {
            ...
            $cell->setValueExplicit($value, \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_INLINE);
