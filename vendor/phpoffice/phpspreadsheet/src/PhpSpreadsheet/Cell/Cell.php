...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception as CalculationException;
...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\Date as SharedDate;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Shared\StringHelper;
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\CellStyleAssessor;
...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Table;
...
class Cell implements Stringable
{
     ...
     * Value binder to use.
     */
    private static ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\IValueBinder $valueBinder = null;
    /**
     * Value of the cell.
     */
    private mixed $value;
    ...
    private \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\IgnoredErrors $ignoredErrors;
    /**
    ...
    public function __construct(mixed $value, ?string $dataType, Worksheet $worksheet)
    {
        ...
        // Set datatype?
        if ($dataType !== null) {
            if ($dataType == \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING2) {
                $dataType = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING;
            }
        ...
        $this->ignoredErrors = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\IgnoredErrors();
    }
    /**
    ...
    protected static function updateIfCellIsTableHeader(?Worksheet $workSheet, self $cell, mixed $oldValue, mixed $newValue) : void
    {
        ...
        foreach ($workSheet->getTableCollection() as $table) {
            ...
            if ($cell->isInRange($table->getRange())) {
                $rangeRowsColumns = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Coordinate::getRangeBoundaries($table->getRange());
     ...
     * @throws SpreadsheetException
     */
    public function setValue(mixed $value, ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\IValueBinder $binder = null) : self
    {
     ...
     * @throws SpreadsheetException
     */
    public function setValueExplicit(mixed $value, string $dataType = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING) : self
    {
        $oldValue = $this->value;
        $quotePrefix = \false;
        // set the value according to data type
        switch ($dataType) {
            case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NULL:
                $this->value = null;
                break;
            case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING2:
                $dataType = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING;
            // no break
            case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING:
                // Synonym for string
                if (\is_string($value) && \strlen($value) > 1 && $value[0] === '=') {
                    $quotePrefix = \true;
                }
            // no break
            case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_INLINE:
                // Rich text
                $value2 = StringHelper::convertToString($value, \true);
                $this->value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::checkString($value instanceof RichText ? $value : $value2);
                break;
            case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC:
                if ($value !== null && !\is_bool($value) && !\is_numeric($value)) {
                    throw new SpreadsheetException('Invalid numeric value for datatype Numeric');
                }
                $this->value = 0 + $value;
                break;
            case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_FORMULA:
                $this->value = StringHelper::convertToString($value, \true);
                break;
            case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_BOOL:
                $this->value = (bool) $value;
                break;
            case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_ISO_DATE:
                $this->value = SharedDate::convertIsoDate($value);
                $dataType = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC;
                break;
            case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_ERROR:
                $this->value = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::checkErrorCode($value);
                break;
            default:
    ...
    public function getCalculatedValue(bool $resetLog = \true) : mixed
    {
        ...
        $spill = \false;
        if ($this->dataType === \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_FORMULA) {
            try {
                $currentCalendar = SharedDate::getExcelCalendar();
                SharedDate::setExcelCalendar($this->getWorksheet()->getParent()?->getExcelCalendar());
                            ...
                            foreach ($resultRow as $resultValue) {
                                if ($row !== $newRow || $column !== $newColumn) {
                                    ...
                                    if ($thisworksheet->getCell($newColumn . $newRow)->getValue() !== null) {
                                        if (!\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Coordinate::coordinateIsInsideRange($oldAttributesRef, $newColumn . $newRow)) {
                                            $spill = \true;
                                            break;
                                        }
                                    }
    ...
    {
        return $this->dataType === \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_FORMULA && $this->getStyle()->getQuotePrefix() === \false;
    }
    /**
     ...
     * @throws SpreadsheetException
     */
    public function getDataValidation() : \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataValidation
    {
        if (!isset($this->parent)) {
     ...
     * @throws SpreadsheetException
     */
    public function setDataValidation(?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataValidation $dataValidation = null) : self
    {
        if (!isset($this->parent)) {
    ...
    {
        $validator = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataValidator();
     ...
     * @throws SpreadsheetException
     */
    public function getHyperlink() : \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Hyperlink
    {
        if (!isset($this->parent)) {
     ...
     * @throws SpreadsheetException
     */
    public function setHyperlink(?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Hyperlink $hyperlink = null) : self
    {
        if (!isset($this->parent)) {
    ...
    public function isMergeRangeValueCell() : bool
    {
        if ($mergeRange = $this->getMergeRange()) {
            $mergeRange = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Coordinate::splitRange($mergeRange);
    ...
    {
        [$rangeStart, $rangeEnd] = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Coordinate::rangeBoundaries($range);
        // Translate properties
        $myColumn = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($this->getColumn());
        $myRow = $this->getRow();
    ...
    public static function compareCells(self $a, self $b) : int
    {
        if ($a->getRow() < $b->getRow()) {
            ...
            return 1;
        } elseif (\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($a->getColumn()) < \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($b->getColumn())) {
            return -1;
        }
        return 1;
    }
     ...
     */
    public static function getValueBinder() : \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\IValueBinder
    {
        if (self::$valueBinder === null) {
            self::$valueBinder = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DefaultValueBinder();
        }
     ...
     */
    public static function setValueBinder(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\IValueBinder $binder) : void
    {
    ...
    public function getIgnoredErrors() : \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\IgnoredErrors
    {
    ...
    {
        if ($this->getDataType() !== \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_FORMULA) {
            return \false;
        }
        $protected = $this->parent?->getParent()?->getProtection()?->getSheet();
        if ($protected !== \true) {
