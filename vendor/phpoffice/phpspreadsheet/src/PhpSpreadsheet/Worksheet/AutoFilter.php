...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Internal\WildcardMatch;
...
use MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column\Rule;
use Stringable;
class AutoFilter implements Stringable
{
     ...
     */
    private ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $workSheet;
    /**
     * Autofilter Range.
     */
     ...
     */
    public function __construct(AddressRange|string|array $range = '', ?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet = null)
    {
        if ($range !== '') {
            [, $range] = \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::extractSheetTitle(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Validations::validateCellRange($range), \true);
        }
        $this->range = $range ?? '';
     ...
     */
    public function getParent() : null|\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet
    {
     ...
     * @return $this
     */
    public function setParent(?\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet = null) : static
    {
     ...
     * @param string $column Column name (e.g. A)
     */
    public function getColumn(string $column) : \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column
    {
        $this->testColumnInRange($column);
        if (!isset($this->columns[$column])) {
            $this->columns[$column] = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column($column, $this);
        }
     ...
     */
    public function getColumnByOffset(int $columnOffset) : \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column
    {
        [$rangeStart, $rangeEnd] = Coordinate::rangeBoundaries($this->range);
     ...
     * @return $this
     */
    public function setColumn(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column|string $columnObjectOrString) : static
    {
        $this->evaluated = \false;
        if (\is_string($columnObjectOrString) && !empty($columnObjectOrString)) {
            $column = $columnObjectOrString;
        } elseif ($columnObjectOrString instanceof \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column) {
        ...
        if (\is_string($columnObjectOrString)) {
            $this->columns[$columnObjectOrString] = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column($columnObjectOrString, $this);
        } else {
    ...
    protected static function filterTestInCustomDataSet(mixed $cellValue, array $ruleSet) : bool
    {
        ...
        $returnVal = $join == \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column::AUTOFILTER_COLUMN_JOIN_AND;
        foreach ($dataSet as $rule) {
            ...
            switch ($join) {
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column::AUTOFILTER_COLUMN_JOIN_OR:
                    ...
                    break;
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column::AUTOFILTER_COLUMN_JOIN_AND:
                    $returnVal = $returnVal && $retVal;
                    break;
            }
        }
     ...
     * @return mixed[]
     */
    private function dynamicFilterDateRange(string $dynamicRuleType, \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column &$filterColumn) : array
    {
        $ruleValues = [];
        ...
        return ['method' => 'filterTestInCustomDataSet', 'arguments' => ['filterRules' => $ruleValues, 'join' => \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column::AUTOFILTER_COLUMN_JOIN_AND]];
    }
    /**
    ...
    public function showHideRows() : static
    {
        ...
        foreach ($this->columns as $columnID => $filterColumn) {
            ...
            switch ($filterColumn->getFilterType()) {
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column::AUTOFILTER_FILTERTYPE_FILTER:
                    $ruleType = null;
                    $ruleValues = [];
                    ...
                    break;
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column::AUTOFILTER_FILTERTYPE_CUSTOMFILTER:
                    ...
                    break;
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER:
                    $ruleValues = [];
                    foreach ($rules as $rule) {
                        ...
                        if ($dynamicRuleType == Rule::AUTOFILTER_RULETYPE_DYNAMIC_ABOVEAVERAGE || $dynamicRuleType == Rule::AUTOFILTER_RULETYPE_DYNAMIC_BELOWAVERAGE) {
                            ...
                            $columnFilterTests[$columnID] = ['method' => 'filterTestInCustomDataSet', 'arguments' => ['filterRules' => $ruleValues, 'join' => \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column::AUTOFILTER_COLUMN_JOIN_OR]];
                        } else {
                            //    Date based
                    ...
                    break;
                case \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column::AUTOFILTER_FILTERTYPE_TOPTENFILTER:
                    $ruleValues = [];
    ...
    public function autoExtendRange(int $startRow, int $endRow) : int
    {
        if ($startRow === $endRow && $this->workSheet !== null) {
            ...
            foreach ($rowIterator as $row) {
                if ($row->isEmpty(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\CellIterator::TREAT_NULL_VALUE_AS_EMPTY_CELL | \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\CellIterator::TREAT_EMPTY_STRING_AS_EMPTY_CELL) === \true) {
