...
namespace MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter;
...
class Column
{
     ...
     */
    public function getRule(int $index) : \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column\Rule
    {
        if (!isset($this->ruleset[$index])) {
            $this->ruleset[$index] = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column\Rule($this);
        }
     ...
     */
    public function createRule() : \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column\Rule
    {
        $this->setEvaluatedFalse();
        if ($this->filterType === self::AUTOFILTER_FILTERTYPE_CUSTOMFILTER && \count($this->ruleset) >= 2) {
            throw new PhpSpreadsheetException('No more than 2 rules are allowed in a Custom Filter');
        }
        $this->ruleset[] = new \MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column\Rule($this);
     ...
     * @return $this
     */
    public function addRule(\MenuManager\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column\Rule $rule) : static
    {
        $this->setEvaluatedFalse();
        $rule->setParent($this);
        $this->ruleset[] = $rule;
